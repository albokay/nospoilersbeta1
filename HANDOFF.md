# Sidebar — Technical State (2026-05-10)

> **2026-05-10 — Latest landed: V2 profile page deep refactor** (left-justified header / bio inline-editable / per-card "remove from profile" trash + modal / TSP filter / double-col Stopped Watching / "remove entirely vs Stopped" choice in V3 stop-watching modal), **V3 close-show / stop-watching ported from v2** (split into "Close show tab" + "Stop watching" with cascade), **V2 chrome unified with AppShell** (logo / sign-out / profile pill render at identical positions across `/v3/journal` and `/v2/*`), **Tier 1 perf pass** (localStorage hydrate-then-refresh for journal first-paint + code-split route components, dropping main bundle from 264 → 219 KB gzip), and **compose-page hover-prefetch + fade-in**. The bio-column migration was applied to prod on 2026-05-10. See §7 arcs (top of recent-work list).
...
28. **`auth.tsx` `loadProfile` + `fetchPublicProfileByUsername` are bio-tolerant** ([auth.tsx:23](src/lib/auth.tsx:23) + [db.ts:1430](src/lib/db.ts:1430), commit `7650b27`). Both try a SELECT including `bio` first, fall back to a legacy SELECT without bio if the first fails (column doesn't exist). Reason: the bio-column code shipped in `cb238c1` but the migration ran shortly after — until the migration applied, the SELECT failed → `data=null` → `setProfile(null)` → /v3/journal and /profile rendered blank because both routes are gated on `profile.username`. The fallback unblocked. **Convention going forward:** any new column-bearing select in shared loaders (anything called from AppShell-level effects) should be try-with-fallback so a code-first deploy doesn't break primary surfaces. Removing the fallback once we're confident every env has the column is safe but optional — the fallback adds one failed query per failure case and is harmless to keep.
