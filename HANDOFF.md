...
> **⏭️ OPEN ARCS / NEXT SESSION — MOBILE IS THE LIKELY NEXT STEP (Alborz will continue in a fresh Code session).** The 2026-07-06→07 work (below) — the whole **social-onboarding arc (CP1–CP6)** + the **contact-naming arc** + the **star rating cells** + all the polish rounds — is **DESKTOP ONLY** and shipped. Mobile has NOT been touched and the gap is the main outstanding work.
> - **1. MOBILE MIRROR ARC (big; likely next).** `/m` (`MobileApp`, `MobileDashboard`, `MobileGroupRoom`, `MobileShowRoom`, `MobileGroupInviteAccept`, sheets) still runs the **pre-restructure opt-in model**: a personal mobile dashboard, global remove-from-pool, the old fill tiers, no proposals/promotion, no 3-screen onboarding, no gated-entry stubs, no per-room leave, no drafts relabel, no contact naming, still dice cells. Known **accepted divergences live right now** (flagged through the arc): mobile shares `computePill` so its shelves + the new green/cream/outline FILL already shifted (a desktop-proposed show reads "no" on the mobile toggle; mobile vote-no still calls the global `remove_show_from_pool`). Server side is mostly ready — the group-scoped RPCs (`get_group_dashboard`, `start_show_room`, `leave_show_room`, contact-name accept seeding) are already live and viewer-agnostic, so mobile mostly needs the **frontend** mirrored. Reuse the desktop sources: `groupPills.ts`, `groupNames.ts`, `SocialOnboarding.tsx` pattern, `StarFace.tsx`, `dashboardChrome.ts`, `db.ts` helpers.
> - **2. BETA-USERS CUTOVER MIGRATION (separate, destructive-ish, needs a backup + go/no-go).** Move the 5 beta users' now-invisible **personal shelves** + **private-journal entries** into the groups-only world — **NOTHING may be deleted**. Not started. Private entries should surface as **drafts** (CP6). See `[[project_social_onboarding_arc]]` locked answer #2.
> - **3. SMALLER FOLLOW-UPS:** (a) `RatingCaptureModal` still shows **dice dots** — inconsistent with the map's stars now; swap to `StarFace` (quick). (b) The **onboarding flow has no "hi, it's…" field**, so its invitees start with the inviter's handle (the invite MODAL has the field + the accept-time seeding works — just wire a field into `SocialOnboarding` Screen 2 if wanted). (c) **Progress-aware onboarding prompts** (haven't-started vs further-along) — spec-deferred. (d) **Naming the person who invited YOU** beyond the "hi, it's…" seed — the gear rename covers it now, but there's no dedicated "rename your inviter" surface.
> - **⚠️ VERIFY MIGRATIONS APPLIED (don't trust "applied" claims — `[[reference_rpc_audit]]`):** this session created `20260706_group_scoped_proposals`, `20260706_contact_names`, `20260706_social_onboarding`, `20260706b_accept_ensures_progress`, `20260706c_leave_show_room`, `20260707_inviter_name_to_invitee_contacts`. Alborz was applying them as we went; the last two (`…c_leave_show_room`, `…07_inviter_name…`) are the ones to double-check on prod.
