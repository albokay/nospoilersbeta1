...
**V2 friend room follow-ups** (from the 2026-05-15 arc):
...
- **Aesthetic pass.** Open notes from initial walkthrough — to be addressed when prioritized.
...
- **Username byline click-to-profile in V2RoomFeed.** Live ShowSection's friend-room thread card uses the `Username` component (underlined name, click → profile). V2RoomFeed currently renders the byline username as plain bold text (no click). Drop-in upgrade if Username's `onClickProfile` is plumbed through. Not a blocker; the whole-card click → thread view is the primary action.
