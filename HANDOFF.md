# Sidebar — Technical State (2026-07-09)
...
> - **0. ⭐ NEXT PLANNED ARC — FIRST-NAME IDENTITY (retire the @handle as a display name).** Spec written + LOCKED 2026-07-09: `~/Downloads/sidebar_display_name_spec.md`. Everyone (incl. the signed-in user) appears by a self-chosen **first name**, never an `@handle`; the handle demotes to an invisible auto-generated pool-URL slug (verified vestigial — login is email+pw, all data keys on `user_id`/email, handle's only jobs are `/pool/:username` + the display fallback). Model: resolution chain becomes `contactName → display_name → slug`; signup collects a first name (no handle to invent) + auto-slug; **"hi, it's…" dropped everywhere** (the signup copy carries the naming idea now); self-view = "(you)" in roster/map, name in bylines; pool URL keeps `/pool/:slug`; ~5 existing accounts backfilled via one SQL UPDATE; impersonation accepted-for-now (first names not unique). 5 CPs: data+signup → **fallback-chain sweep (the big one, contact-naming-arc breadth)** → self-view → set/edit + drop hi-it's → optional name-prompt. ⚠️ verify the prod signup trigger first (untracked; `[[reference_rpc_audit]]`). Fresh session.
