# Sidebar — Technical State (2026-06-06)
...
- **Account deletion** — full user data purge with care for shared catalog (TSP seeds etc.). See §6 item 25 for the gotcha pattern (don't sweep up `is_seed=true` author content via thread-scoped joins).
  ...
  **Test-user convention for onboarding testing (decided 2026-06-06, not yet built).** Email confirmation appears to be OFF (signUp has no "check your email" step for signup and expects immediate login — verify at Auth → Providers → Email before relying on it). With it off, onboarding can be tested with made-up emails, no real inbox: each signup is a genuine fresh new-user (gets its own TSP room via the provisioning trigger). **Convention:** every test account uses a single DEDICATED fake email domain (e.g. `001@sidebar.test` — the `.test` TLD is reserved for this; `@example.com` also valid) with a matching `001sidebar`-style username + reused password. The **email domain is the cleanup key** — collision-proof (no real tester uses it) and lives on `auth.users` (the deletion root), so "find all test users" = `select id from auth.users where email like '%@sidebar.test'`. NOTE the proposed `001sidebar.sidebar` form is INVALID (no `@`/TLD) and Supabase rejects it — must be a real email format. The convention makes *finding* test users trivial; *deleting* them safely is the same shared-catalog-aware cascade as the account-deletion item above (scope by `author_id` + the user's own `group_id`s, never by `thread_id`; remove profile + `auth.users` row last via service role). Building a dev-only "create fresh test user" + "safely delete user" pair would serve onboarding testing AND move the account-deletion + Confirm-email checklist items forward at once.
