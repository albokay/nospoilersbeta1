...
15. **Profile-load failure leaves signed-in non-admin on homepage** (`b0fe122`). The `/` → `/profile` redirect is gated on `user && profile && !isAdmin`. If the profile row never loads (network error, RLS failure, trigger race, or a beta-prep SQL reset that deleted the user's `auth.users` + `profiles` rows while the JWT persists in localStorage — i.e. "dangling token" state), the redirect never fires and the user stays on `/` viewing the anonymous homepage. Not an infinite loop and not a blank page — just a degraded state. **Mitigation (`e8bc94c`):** the Sign-out button on both auth clusters now renders on `user` alone (previously `user && username`), so a dangling-token user can always sign out and re-sign-in rather than being trapped. Before this fix, the only escape was clearing localStorage via DevTools. If the redirect-never-firing symptom is observed in practice after this mitigation, add a "still loading your profile… [retry]" fallback or stop gating the redirect on `profile`.
