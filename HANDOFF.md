...
> **2026-05-16 — Latest landed: rating capture modal (V2/V3 progress-update flow).** A new modal replaces the existing red/white "you've watched: SE" confirm on every forward progress pick at the 3 V2/V3 picker callsites — V2FriendRoomPage, V3 show-tab header, V2ComposePage's own picker. Six rating pills + cancel, tap-to-commit with a 150ms label-collapse on the non-selected pills. Picks on A+B (V2FriendRoomPage + V3 header) carry the user into `/v2/compose` with rating-flow markers — intro copy variant ("Get your first thoughts down...") + returnTo-aware discard so they land back where they started. Picks on D (V2ComposePage's own picker) capture the rating + advance progress without leaving the draft. V2 compose post-publish nav is now destination-driven for ALL publishers (rating-flow + non-rating): private → `/v3/journal` with the private radio segment pre-selected, public → `/show/:id`, friend room → `/v2/room/:gid`. Six commits + one SQL migration to expand `episode_ratings.rating` from 1..5 → 1..6 (added "Solid." + "I'll keep going."; shifted "Losing me." 4→5, "Nope." 5→6). V1 callsites (ShowSection ×4, ProfilePage ×2) intentionally untouched — they keep the legacy confirm. See §7 arc.
>
> **Prior arc landed earlier today (2026-05-16):** V2 inline thread polish pass — quote button ported from v1 verbatim (selection-based, `Quote…` label, hint modal), composer made click-to-open with cancel/submit closing it, star moved up to the title row so it doesn't jump between collapsed/expanded states, the per-reply `Respond` buttons + bottom "Respond to the thread" CTA suppressed via a new opt-in prop on `RepliesList` (a 3-line additive touch — v1 unaffected), single large "Write a response" CTA below replies, two collapse buttons styled as plain white text, the bottom-right "expand" affordance reduced to a single white chevron icon, scroll-to-top anchored 72px below viewport via `scroll-margin-top`, map filtered to only "touched" seasons (≥E1 reached), tombstones faded to 0.35 opacity, and map's on-mount scroll-to-progress effect removed.
>
...
- **Delete `V2JournalPage.tsx` + drop `/v2/journal` from V2App's router** — superseded by `/v3/journal` since 2026-05-09. The old V2JournalPage is unreachable but still in the bundle. Once you've confirmed `/v3/journal` is the keeper, remove.
- **Reconsider whether `/v3/journal` becomes the canonical `/profile`.** V3 was a duplicate of live ProfilePage to allow free iteration; substantial divergence has now happened (write button → /v2/compose, close-show modal, profile cross-link). When ready, make `/profile` an alias for `/v3/journal` (or fold V3 changes back into ProfilePage and delete the duplicate). Defer until v3 is stable enough to be the primary surface.
