...
21. **Supabase Security Advisor pass 2026-04-23 — deferred findings.** Manual dashboard review after the read-only audit arc. 2 info, 14 warnings, 0 errors. Dispositions:
    ...
    **Priority order when this becomes actionable:** ~~(1) `friend_groups` SELECT policy~~ — **DONE: verified not a leak 2026-06-03, see above.** (2) Leaked Password Protection — pair with pre-beta email coordination. (3) Function search_path cleanup — cosmetic, whenever. (4) `thread_prompts` policy — probably skip unless priorities shift.
