...
>   - **(a) Invite DECLINE has no surface** since the "You're in!" card became the accept confirm — a recipient can back out (dim-click) but never refuse/delete an invite; a red pending cluster clears only by joining. Awaiting Alborz's call on a quiet "no thanks".
>   - **(b) Invitee gets 8 cards nearly back-to-back** — wave 1 right after GET STARTED!, then landing in the room immediately satisfies the wave-2 trigger. Small gate change if wave 2 should wait for the next session.
>   - **(c) Negated restatement forms unauthored** — so hot takes are solo-YES-only and the unanimous headline fires on all-YES cards only (`src/lib/deckFindings.ts`; spec §7.5.8's example hand-wrote one negation). Add a `negated` column + authoring pass to unlock solo-NO.
>   - **(d) Ally/opposite TIE copy** (spec §7.2 TODO) — ties currently break silently deterministic in deckFindings.ts.
>   - **(e) §4.6 "both axes" cards seeded OFF-AXIS (0/0)** (speed-15x, second-screen, phone-during-tv, missed-plot-point) — Alborz is owed a plain-language explanation + his axis call BEFORE the map ships.
>   - **(f) Drag-to-swipe gesture** — question cards are tap-only (NOPE/YES tabs); polish item.
>   - **(g) TSP demo `?` re-entry link** — the demo left the onboarding flow (dormant behind `?tspdemo=1`); the planned dashboard/room `?` link doesn't exist yet.
>   - **(h) Mobile edit gap at ≤8 answers** — "see more answers →" (the only route to the mobile grid + its edit pencil) appears only past 8 answered, so early mobile users can't edit answers on mobile (desktop works). Revisit if testers hit it.
>   - **(i) Cross-device confirm-email edge (accepted)** — the invitee's 4 pre-wall answers live in the first browser's localStorage; confirming email on another device loses them (they re-answer via normal waves).
>   - **(j) DEFERRED spec surfaces:** the map (16-axis-card gate; quadrant names + dead-centre copy deliberately wait for real data), the n≥4 personal card, the admin yes-rate surface (meanwhile: the SQL query at the bottom of `docs/swipe-deck/deck_cards_seed_DRAFT.sql`), the digest "answered more questions" line.
> - **0. ✅ FIRST-NAME IDENTITY ARC — COMPLETE 2026-07-10 (CP1–CP4 + CP6 shipped same-day; live-site confirmed by Alborz; full dated entry below).** The @handle is RETIRED as a display name: everyone (incl. the signed-in user) appears by a self-chosen **first name**; the handle survives only as an invisible auto-generated pool-URL slug + internal keys. Resolution chain everywhere, both platforms: `contactName → display_name → bare handle` (last link effectively never fires — all 7 real accounts backfilled + signup requires a name; the "@" prefix no longer renders anywhere live). Spec + 07-10 amendments + the **PUBLIC ROOMS future-context section** (claim-a-vanity-handle = rename of the auto-slug at first public act; friends' contact names win even in public; old /pool URLs may 404 on rename — accepted): `~/Downloads/sidebar_display_name_spec.md`. **Parked:** CP5 one-time name prompt for null-name accounts (can't matter today — a nameless account can't come into existence). **Public-rooms readiness landed:** avatars + touched identity keys anchor on `user_id`, so a future handle rename can't change anyone's avatar or confuse sessions.
> - **1. ~~BETA-USERS CUTOVER MIGRATION~~ — RETIRED 2026-07-07 (unnecessary; evidence-based, Alborz's call).** A read-only audit of prod found: the ONLY group-linkless private writing in the system is Alborz's own 3 test entries (paradise ×2 + thebear ×1), and both shows are ALREADY reachable as drafts through his existing rooms (the drafts tab reads the viewer's private threads per show whenever any of their groups has a room for it — "no group link" ≠ unreachable); the orphaned personal-shelf rows are 11 unimportant shows across 4 accounts. **No migration will be written; NOTHING deleted** — the progress rows stay dormant, and the built-in recovery covers them forever: searching a show in any group proposes it with the saved progress restored. If a tester ever asks "where did X go," the answer is "search it in your group."
> - **2. ~~POOL PAGE LEAKS GROUP PROPOSALS~~ — RESOLVED 2026-07-07 by REFRAMING (Alborz decision): the pool/invite surfaces are now OPT-IN-BASED.** Listing proposals is deliberate (another user can ask to watch together) — the fix was honest copy + a clean definition, not hiding. All THREE surfaces (`/pool`, `/m/pool`, both invite arrivals) share one logic + copy: shelf 1 **"[name] is interested in starting these shows:"** = live yes-votes in any of their groups (proposing = yes; EXCLUDES rooms they deliberately left — an exit shouldn't re-advertise interest); shelf 2 **"and is already watching these:"** (solo variant "[name] is already watching these shows:") = shows with an open room they're a current member of (incl. s0e0 auto-opted invitees, by design). **Watch progress ALONE no longer surfaces a show** — deliberate + consistent with the group model ("just confirm my progress" = non-committal; the 5 beta users' pools read EMPTY until the cutover migration, item 1). **Migration `20260707b_get_public_pool.sql` (⚠️ APPLY TO PROD):** new anon-callable SECURITY DEFINER `get_public_pool(target_user_id)` → (show_id, bucket 'room'|'proposal', season, episode) — exposes only that; no group names/co-members/counts. Frontend tolerant pre-migration (RPC missing → the old progress-derived shelves with the NEW copy).
> - **3. SMALLER FOLLOW-UPS** *((a)+(b)+(f) DONE 2026-07-07 — the quick-wins batch)*: (a) ~~RatingCaptureModal dice~~ → **StarFace in a sky circle** (local `RatingStars` wrapper; both platforms — the pick-a-rating choices now match the map). (b) ~~no "hi, it's…" in onboarding~~ → the optional field was added 07-07, then **the whole "hi, it's…" mechanism was RETIRED 2026-07-10 by first-name-arc CP4** (the invite email introduces the inviter by their profile first name; no typed field anywhere). (f) ~~MobileSearchSheet dead personal mode~~ → stripped (`outOfPool`/`onRestore` props + restore branch gone; two modes remain: groupContext = propose, bare = onboarding pick). **Still open:** ~~(c) progress-aware onboarding prompts~~ — **DONE 2026-07-08** (⚠️ needs Alborz's one-time SQL insert if not yet run): 10 Alborz-written prompts for prod with NEW tags `onb-fresh` (declared s0e0–s1e4) / `onb-returning` (s1e5+) — tags the regular composer never computes, so they're invisible outside onboarding; new `ComposeForm.promptPoolTag` opt-in serves ONLY the tagged batch and CYCLES it when exhausted (never the regular pool; empty batch degrades to normal); both onboardings pass the tag from the declared progress. Prompts are DB-only (not in promptData seed — admin-created precedent, no id-drift). (d) ~~naming the person who invited YOU~~ — **CLOSED 2026-07-10 by first-name-arc CP6** (the received-invite cluster names people by first name); (e) **mobile group rooms have no `GroupRoomSticky`** (predates the arc), so the CP3 bootstrap "we went ahead…" note shows only if the group is ever opened on desktop (`ns_onb_group` is set by mobile onboarding too).

> **2026-07-17→20 — SWIPE-DECK ARC (CP0–CP4 + 5 QA rounds) + INVITEE-FUNNEL CHANGESET + MULTI-FRIEND ONBOARDING + PENDING-INVITES CHANGESET — ALL SHIPPED.** Spec `~/Downloads/sidebar_swipe_deck_spec.md` (v0.1 + in-chat amendments); standing issues consolidated in the open-arcs block at the top of this doc. The feature: a ~60s agree/disagree deck about HOW you watch TV → a per-group "How We Watch TV" answer grid + per-person "How I Watch TV" card, docked bottom-of-page both platforms; artifact incomplete without friends = invite pressure. **Locked in review:** titles are **"How I/We Watch TV"** SITE-WIDE (amends spec §11.5's "How I/We Watch"); page DIMS behind every card surface incl. the drip modal (supersedes the undimmed group-room mockup); mobile = docked→slide-open sheets, Lucide thumbs (personal-green up / alert-red down) instead of pills/dots, group-card friend names rotated vertical-CCW, full-grid cells carry a cream thumb; answers-led artifact + grid-behind-tap per §12.10 (mockups approved rev 2, `docs/swipe-deck/mobile-result-surfaces-preview.html`). **Arc decisions from Q&A:** deck+waves ship BOTH platforms; Findings sticky ships this arc (desktop-only per spec); n≥4 personal card / map / admin yes-rate surface / digest line deferred; drip = GLOBAL calendar `released_at <= now()` (first time-gate in the codebase), 4-per-login catch-up, NO dismissal (must answer, same no-skip as onboarding); the onboarding 8 is fixed for everyone forever; existing accounts catch up via the same drip modal; wave-2 trigger = onboarding COMPLETION (inviter) / FIRST group-room click (invitee); waves take precedence over a queued drip; onboarding flow changes ride this arc (land on DASHBOARD not in-group, TSP demo out of the flow — later `?` re-entry link, "You're in!" reformatted as a card, inviter+invitee variants, mockups in hand). **⚠️ PINNED for Alborz:** the four §4.6 "both axes" cards are seeded off-axis (0/0) — needs a plain-language explanation + his call before the map ships (map is ≥1 drip away, not urgent).
> - **CP0 (no code).** 3-agent convention research → schema/RPC/drip proposal APPROVED. Seed `docs/swipe-deck/deck_cards_seed_DRAFT.sql` APPROVED (apply AFTER the CP1 migration; **EDIT v_launch at the top first**): 59 cards (the spec's §4 tables hold 59, not "~53"), onboarding 8 = phone-during-tv / avoid-trailers / three-episodes / spoiled-by-thumbnail + lied-caught-up / quietly-finished / fall-asleep / full-season; ALL card_types are initial guesses (40 split / 13 confession / 6 consensus), correctable one UPDATE at a time off the admin yes-rate query at the file's bottom (covers the spec §2 empirical-classification need with zero build); 13 drip batches ≈ 26 weeks runway, poles mixed per batch.
> - **CP1 data layer.** Migration `20260717_swipe_deck.sql` (**APPLIED 2026-07-17, then the seed**): `deck_cards` (prompts-style RLS — released+active readable by all, UNRELEASED admin-only so the drip can't be read ahead via REST; admin-only writes) + `deck_answers` (episode_ratings clone: owner-only RLS ×4, uuid PK + UNIQUE(user_id, card_id), updated_at touch trigger; answers are GLOBAL per user, not per-group — one answer feeds every group's grid) + `get_group_deck_answers(p_group_id)` (LANGUAGE sql STABLE SECURITY DEFINER, people_group_members-gated, released+active cards only, empty result for non-members — the ONLY cross-user path). db.ts additions: `DeckCard`/`GroupDeckAnswer` types + `rowToDeckCard`, `fetchDeckCards` (server-side `lte(released_at, now)` so ADMIN sessions see the same deck as users despite the RLS admin override), `fetchMyDeckAnswers` (cardId→bool map), `upsertDeckAnswer` (onConflict user_id,card_id; NO rate limit — boolean-only, mirrors upsertEpisodeRating; THROWS so callers can revert optimistic state), `fetchGroupDeckAnswers`; all three reads are pre-migration tolerant ([]/{}, warn-and-continue) so nothing can break before Alborz applies the SQL. Artifact math (agreement counts, hot takes, n=2 header, Findings selection) will be CLIENT-SIDE arithmetic over the one RPC read per spec §9, counting only cards BOTH people answered (drip desync can't skew); grid edit-mode saves will batch through Promise.all like `commitRatings`.
> - **CP2 waves + flow rework (`1813383`).** `DeckWave` (shared question card: cream, Lora Identity statement; desktop NOPE/YES side tabs breaking the edges, mobile corner tabs NOPE top-left / YES bottom-right; self-skipping — mounted anywhere, it silently completes when nothing is owed). Both onboardings open with WAVE 1 (§12.4 welcome copy) and close with WAVE 2 ("a few more…") + the "You're in!" card (`YoureInCard`, inviter/invitee variants per the mockups); onboarding lands on the DASHBOARD (was ?g=); TSP demo OUT of the flow (dormant behind `?tspdemo=1`); invite accepts serve wave 1; wave 2 = first group-room entry (`requirePriorWave` keeps wave order for catch-up accounts).
> - **CP3 desktop results (`9e3efe2`) + Findings copy pass (`4515bc0`).** Docked "How I/We Watch TV" cards (base dashboard = your column; group room = everyone's) → open centered over the dim: frozen panes (statements + (me) sticky-left, header sticky-top), edit mode (pencil → your cells shrink to chips w/ the room map's two-phase bounce; confirm check rides the frozen header; batch save = the `commitRatings` pattern, patch-in-place, revert-safe), n=2 inline header (§5 — high ≥75% of ≥4 common, stark ≤25% of ≥8, else plain), n≥3 **"Findings:" sticky** (desktop-only, StickyNote shell; generate on open / clear on edit-entry / regenerate on confirm). **`src/lib/deckFindings.ts` = ALL the artifact arithmetic AND every copy template in one block** — Alborz rewrote the renegade + can't-agree headlines and the ALIGNED check was reordered BEFORE unanimous (the old order made the §8 ending unreachable). Agreement counts only cards BOTH people answered.
> - **CP3b + CP4 (`2139ba4`).** Mobile results (approved rev-2 mockup `docs/swipe-deck/mobile-result-surfaces-preview.html`): docked → slide-open sheets, Lucide thumbs (personal-green up / alert-red down), vertical-CCW friend names, full grid behind "see more answers" w/ cream thumbs in cells + frozen panes + the same edit mode. Drip/catch-up modal on ALL main surfaces both platforms: ≤4 released-unanswered cards oldest-first, once per session (**stamped on batch COMPLETION, so reload isn't an escape**), wave-2 cards EXCLUDED from the drip (their reserved moments can't be preempted), waits for the onboarding gate to resolve (waves take precedence).
> - **QA rounds 1–5 (`81523f7` `0a2ed79` `a073424` `d27dcd4` `fa5aa16`).** "Edit answers?" tooltip + chip shrink + bounce; answer exits slide toward the chosen tab; Sidebar logo OFF all result surfaces (Alborz reversed the §11.5 in-card-mark rule); flush grid right edges (last column flex-grows); flat shadowless tabs incl. the stadium GET STARTED!; "a few more…" in Header 2; baseline-aligned headers, grid divider removed; docked = TITLE ONLY (Personal green on dash / Sky in room; Identity when open); mobile sheet names share the header band; "see more answers →" only past 8 answers; mobile corner tabs + the wave overlay anchored below the page chrome; **the "You're in!" invitee card BECAME the invite confirmation** (GET STARTED! = accept → wave 1 → STRAIGHT INTO the group room; the Yes/no modal is gone — see standing issue (a)); `lied-caught-up` statement copy UPDATE run on prod.
> - **Release-date incident (caught + fixed same day):** the seed was applied with the placeholder `v_launch` (2026-07-20) → 0 cards released and every surface correctly hid the deck; fixed by a −3-day shift UPDATE (launch = 2026-07-17). Prod read-only verification via the CLI keychain token works — the keychain value is `go-keyring-base64:`-prefixed (base64-decode before use with the management API).
> - **INVITEE-FUNNEL CHANGESET (spec `~/Downloads/sidebar_onboarding_changeset.md`; 3 CPs, 2026-07-20).** CP1 SQL (`docs/swipe-deck/onboarding_changeset_CP1.sql`, APPLIED — self-anchoring, no date editing): NEW card **`just-wait-ep4`**; the first-8 re-selected to 3 spoiler-ethics + 5 confessions (**deliberate §12.8 reversal** — confessions are the hook now); DB sort = the INVITER serve order (`phone-during-tv` opens), the invitee path pins `just-wait-ep4` first client-side (`DeckWave.leadCardId`); displaced avoid-trailers / three-episodes / lied-caught-up / fall-asleep / full-season → drip, one per batch (+2w…+10w) + clean renumber. CP2 (`1ed6e85`): **PRE-WALL WAVE** — the logged-out invitee answers wave 1 at the door before any account (`DeckWave.anonymous`; answers park in localStorage `ns_deck_pending` via `src/lib/deckPending.ts` and are claimed with OVERWRITE by the next sign-in — AuthProvider effect); completing reveals the pool-welcome + JOIN IN wall. CP3 (`24eb3ee`; fn REDEPLOYED): the invite email leads with the inviter's OWN yes/no on `just-wait-ep4` ("…thinks that "just wait for episode 4" when discussing TV counts as a spoiler." / "…is not a spoiler.") + bold "Do you?"; static-headline fallback when the inviter hasn't answered it; body ends "…so you can say what you actually think."; tagline + group-name closer cut; CTA "Join in →".
> - **MULTI-FRIEND ONBOARDING (`61d1cd8`).** Screen 2 = "Who's at least one friend you always text about TV?"; a cream "+" circle (Sky plus) adds rows, **cap 7** (= the 8-MEMBER room cap minus you — checked at invite-mint AND accept; Alborz kept the 8 cap); per-friend invites/emails with per-friend copy-link fallback rows; the "You're in!" card joins the names.
> - **PENDING-INVITES CHANGESET (spec `~/Downloads/sidebar_pending_invites_changeset.md`; CP1 `0837912` + CP2 `052894f`; migration `20260720_pending_invite_nudges.sql` APPLIED; fn REDEPLOYED).** Gear "Pending invites" panel on both platforms — the viewer's OWN invites only: name/email + "invited N days ago"; **nudge** = hidden-until-clicked prefilled editable textarea ("Hey {name}, I invited you to watch shows on Sidebar, still hoping you'll join.") → same-channel email (subject "{inviter} still hopes you'll join them on Sidebar.", text above an untouchable Join in → button; ≤500 chars; creator-only; a DELIVERED nudge sets `last_nudged_at` + renews `expires_at` to +7d, so **expired invites revive on nudge**); **rescind** = one inline confirm → creator-gated `rescind_people_group_invitation` (delete — the invitee-decline end-state). Signal layer: Identity-blue dot (Sky ring) on the gear at 3 days' silence per invite (nudge resets; accept/rescind ends), desktop cursor tooltip via the existing onTip system ("N friend(s) haven't joined yet.", orphan-safe), and the panel line "…A nudge might be all they need." on both platforms (mobile has no hover). Shared `src/components/PendingInvitesPanel.tsx` (exports `isInviteStale`/`staleInviteLine`, 3-day constant). Nudges ride the existing rate limits (3/invite/24h, 30 emails/day). Gear-only per Alborz — no dashboard-cluster echo yet.
> - **Odds & ends (`57e8ef1`).** Tooltips clear on ANY location change (the stuck-tooltip-after-?g=-navigation bug); docked card titles match the page behind them; dashboard clusters bumped (avatar circles 40→56, cluster name 14→17) — the cluster is a new user's first sight of the product.

> **2026-07-10 — FIRST-NAME IDENTITY ARC (CP1–CP4 + CP6, all shipped + live-confirmed same-day).** Spec `~/Downloads/sidebar_display_name_spec.md` (drafted+locked 07-09; 07-10 amendments inside). 5 commits; 3 prod SQL applications + 1 edge-fn redeploy by Alborz. **New standing rule established during CP1:** Claude verifies prod READ-ONLY via the Supabase CLI keychain token → management-API query endpoint; ALL prod writes (DDL/backfills) are pasted by Alborz into the SQL editor.
> - **CP1 data + signup (`999f696`; migration `20260710_display_name.sql` APPLIED; backfill RUN).** `profiles.display_name` (nullable, NOT unique) + the prod signup trigger `handle_new_user` extended to persist it from auth metadata (`SET search_path` added per convention; the untracked prod body was read FIRST via the mgmt API — matched the assumed shape). Signup on BOTH platforms asks for **"your first name"** (required, ≤40, helper copy per spec); the handle is auto-generated invisibly in `auth.tsx signUp` (slugified name + random suffix, collision-checked, widening retry — replaces the username-taken check); all "username" copy gone from auth surfaces. Backfill: **7 real accounts, not the spec's ~5** (Alborz/Christine/Jill/Libby/Matt + throwaways First/Zrobla; `firstuser2` never existed — it's `firstuser`). `loadProfile` + `fetchPublicProfileByUsername` return `display_name` (column-tolerant, §6 item 28 pattern). Deploy order: migration BEFORE push, so no nameless account could be created.
> - **CP2 the site-wide sweep (`c8102b4`) — ZERO migrations.** Every name surface on both platforms now chains `contactName → display_name → username`: entry/reply bylines, show-room roster/map/member-filter, gated stubs, group chat (incl. realtime inserts), "with…" lines, cluster labels, opt-in avatar letters/tooltips, "Read what X has written?", pool pages (anon visitors see the owner's first name too). Plumbing: `PeopleGroupMember`/`GroupMessage`/`RoomMapMember` carry `displayName` (the room-map RPC predates the column, so `fetchRoomMapData` resolves names in a client-side follow-up profiles read — no prod RPC change, and it covers departed writers who left the group); `personDisplayName` gained an optional 4th arg; page-level `displayNames` maps chain at build time. Gear rename placeholders = the person's first name (a raw slug placeholder would be ugly). Contact renames still override everything; the gear's contact-edit VALUES stay pure contact names.
> - **CP2 amendment — avatar seed → `user_id` (the public-rooms hedge).** `SidebarAvatar` seeds userId-first (was username-string, deliberately — its old header comment predicted this exact tradeoff). Closing the loop required `Reply.authorId` (replies never mapped `author_id`; NULL for anonymized accounts) so reply bylines seed identically to entry bylines — the historical mixed-seed bug can't recur as long as live callsites pass userId (all audited). **Every existing avatar changed appearance ONCE at this deploy** — accepted (cheap at 7 accounts, painful at scale; that was the point of doing it now).
> - **CP3 self-view (`6654efa`).** The map's self column + the mobile roster self row are labeled just **"(you)"** (no name; the map header band no longer stretches for long self-names); member filters say **"only you"**. Own bylines publish the first name via the CP2 chain — verified, no special-casing needed.
> - **CP4 set/edit + "hi, it's…" retired (`5c3e0b3`; ⚠️ `send-group-invite` REDEPLOYED by Alborz).** Account gear (shared `AccountModal`, both platforms) gained a **"Your name"** editor (required-non-blank → the slug can't resurface; saves via new `setOwnDisplayName` + `refreshProfile`) and "Signed in as @handle" → "Signed in as **email**". The "hi, it's…" field + explainer removed from ALL 4 surfaces (desktop invite modal, mobile invite sheet, both onboardings' screen 2); `sendGroupInviteEmail` dropped its name param. The edge fn resolves the inviter's `display_name` itself (chain: profile name → legacy typed param from stale bundles → @handle) and **auto-fills `people_group_invitations.inviter_display_name`** — column KEPT, so the welcome screen + accept-time contact seeding work unchanged (decision: keep-but-auto-fill, no migration). Arrival screens needed no code change.
> - **CP6 received-invite cluster (`afc3366`; migration `20260710b_pending_invites_display_names.sql` APPLIED) — closes open-arc 3(d).** The LAST live @handle surface: `get_my_pending_group_invites`'s payload extended ADDITIVELY (`inviter_id`, `inviter_display_name` = profile name → invite's carried name, `members` [{id, username, display_name}]; legacy keys kept → deployed bundles unaffected). Client chains contact → first name → bare handle via new shared `pendingInviteMemberNames`/`pendingInviterLabel` (groupNames.ts): "You've been invited by Alborz…", "Join a group with Christine and Matt?", cluster letters — both platforms; the "@" is gone from this surface. Urgency context: existing testers had readable handles, but post-CP1 signups have slugs ("@matt-k3f9 invited you") — now moot.

> **2026-07-09 — UI-POLISH BATCH (group-shelf avatars, mobile group-room chrome, show-room header) + display-name spec.** Frontend-only; each build-verified + pushed. (1) **`e610d37`** — desktop group-shelf opt-in avatars pulled up 4px (`optInRow` translateY 50%+8 → +4) now the pen badge is gone. (2) **`76b1c8b`** — avatars: on green (Personal-filled) show pills the NON-writer outline goes Personal-green; writers keep sky outline (a green ring on green fill would vanish); both platforms. (3) **`448be1a`** — mobile group-room actions: equal width sized to the "Add friends" label (stretch column, NOT full-screen), "Add friends" → Identity fill/cream text; + cream avatar letters. (4) **`81653b8`** — avatar letters → cream (desktop group-room + mobile show-room roster). (5) **`9029b7c`** — mobile group room chat is now a right-edge TAB (matches desktop `chatTab`, 60% height); the non-functional shows/chat toggle removed. (6) **`0fb24b1`** — show-room header "with…" lists the show's OPTED-IN room members (from `roomMapData`, non-departed, non-self), NOT the whole group; custom group name still wins; NO "with" line when solo (was "with Group N"); both platforms; dropped 2 per-load member fetches. (7) **FIRST-NAME IDENTITY SPEC** written (`~/Downloads/sidebar_display_name_spec.md`) — see open-arc #0.

> **2026-07-08 — ODDS-AND-ENDS BATCH + writer-avatar restyle + leave-modal copy (all pushed; build-verified; edge fns redeployed).** Frontend + 2 edge functions. ⚠️ **Edge-fn deploys DONE by Alborz:** `send-digests`, `send-group-invite` (+ a harmless re-deploy of unchanged `send-invite`).
> - **Onboarding sticky copy** (`d3bb502`) — the onboarding-variant `GroupRoomSticky` rewritten to Alborz's copy w/ his line breaks ("You have a room started for your first show." → propose/vote/start para → 💬 line → spoiler-gate caveat, italic+muted). Desktop only (mobile still has no sticky — open follow-up (e)).
> - **Mobile show-room "with" header** (`4264dd2`) — the "with [group]" span → cream, Inter bold 14 (Body), matching desktop (was blue `C.blue`).
> - **Inviter's typed name replaces @handle throughout the invitee flow + email** (`11956c1`) — if the inviter filled "hi, it's… [name]", the invitee sees ONLY that name (no @handle, no parens): invite email (subject + bold headline + text), the arrival pool headings (via new `PublicDashboardPage displayNameOverride` — logged-out invitee has no contacts), "Join a group with [name]?", the auth hint. Blank field → @handle fallback everywhere. Both platforms + `send-group-invite`.
> - **Pool/invite: s0e0 room shows → "interested in starting" shelf** (`7981fd7`) — a show with an open room but the viewer at s0e0 now lists under "is interested in starting these shows:" not "already watching" (an inviter's un-started bootstrap show reads correctly). All three surfaces (`PublicDashboardPage`, `MobilePool`, `MobileGroupInviteAccept`); the split filters `pool.rooms` by started, moves the rest into `proposals`.
> - **Digest author names + "S" avatar flash** (`af46c95`) — digest (edge fn) names each author by the RECIPIENT's given (contact) name → else @handle → else "a friend" (never bare "@someone"; the "@someone" was the username lookup missing). Formatting reworked so given-names render clean, handle-fallbacks keep "@" (new `formatNames`, per-recipient `contact_names` bulk read). ALSO: the group-shelf opt-in avatars no longer flash "S" (they defaulted a not-yet-loaded member name to the literal "someone" → "S"); an unresolved member now shows a letterless circle until names load (new `resolved` flag on the OptIn type, both platforms).
> - **Digest unsubscribe instruction** (`d3bb502`) — HTML + text: "…click the ⚙️ next to its name, then choose 'unsubscribe.'" (was the nonexistent "toggle 'email digest' to 'off'"; matches the live button).
> - **Onboarding re-fires at zero groups** (`99473a9`) — the gate no longer early-outs on the `social_onboarded_at` flag; it fires whenever a user has NO groups AND NO pending invites — so a returning user who LEFT all their groups gets re-guided (Alborz: a resetting user shouldn't be stranded on the bare "Create another watch group?" screen). Invited accounts (pending invite, or a group just accepted) still skip; TSP demo stays gated-once so it won't re-appear. `fetchSocialOnboarded` import removed from both dashboards. Consequence (intended): a 0-group user can't linger on an empty dashboard — onboarding takes over until they make a group.
> - **Writer indicator: pen badge → avatar restyle** (`3bc7d78`) — the group-shelf opt-in avatars drop the pencil; a member who's written now shows **Personal(green) fill / Friend(sky) outline / Cream text** (default = sky fill, blue text, cream outline). Both platforms; orphaned `Pencil` import + `writerPencilBadge` cleaned out. Tooltip copy (desktop; mobile has no per-avatar tooltips): "They have writing in here." (was "…started writing…"); every NOT-STARTED member (writer or not) now gets a two-line "(name) hasn't started / watching yet." (was "hasn't started yet"). **Pen/writer trigger reminder:** `wrote` = the member has ≥1 non-deleted ENTRY (not reply) in THIS group's room for the show (`entry_writers` CTE in `20260706_group_scoped_proposals.sql`).
> - **Leave-room modal final line** (`d6ed728`) — "Search and add the show back anytime to rejoin." → "Simply re-propose the show to rejoin the room." Both platforms.
> - **Onboarding screen-2 name/email** (`aef3055`) — now the standard side-by-side invite row ("their name" / "email"), both platforms (a stacked look was briefly tried + reverted before commit).
> - **Onboarding prompt batches** (`ca19ae6`) — 10 Alborz-written prompts (DB-only, tags `onb-fresh` s0e0–s1e4 / `onb-returning` s1e5+) served via new `ComposeForm.promptPoolTag` (cycles the batch, NEVER the regular pool; empty batch degrades to normal). Both onboardings pass the tag from declared progress. Prompts inserted on prod by Alborz (RLS confirmed on prod: enabled + 4 policies).

> **2026-07-07 — PERF PASS: room first-loads parallelized + standard "loading…" line + map edit-save no longer reloads the room (3 CPs).** Frontend-only; build-verified. (1) **`0a3c476`** — ShowRoomPage + MobileShowRoom load chain 5→3 serial legs (drafts fetch joined the main parallel batch; the group-name lookup resolves CONCURRENTLY with `fetchGroupThreads`, error fallback contained in-promise); MobileGroupRoom's personal core + `refreshGroup` now run concurrently (were serialized = doubled first paint); BOTH platforms' `refreshGroup` stale-catalog leg (`fetchShows` + `refreshStaleShows`) is fully non-blocking (was an awaited second full-catalog fetch — also made post-vote re-syncs snappier). Deliberately NOT touched: `fetchShows` caching (the reverted May landmine, §6 item 29). (2) **`db2b89c`** — standard loading line "loading" + `LoadingDots`, Header 2 (Inter 700 14) cream, centered, on every blank/bare loading state: desktop dashboard + show room (were blank green), mobile show room + MobilePool (blank), the lazy-chunk `RouteFallback` (bare dots), and the mobile dashboard/group-room dots spots (standardized). (3) **`e9bc237`** — `commitRatings` (map rating EDIT-MODE save) patches the viewer's own `mapMembers` cells in place after the writes instead of `await load()` (the full room refetch visibly restarted the page); failure path unchanged; the progress-advance rating capture still reloads (genuinely re-filters the feed). Single-cell click-to-rate was already optimistic.

> **2026-07-07 — MOBILE MIRROR ARC: `/m` brought to the desktop social-onboarding world (7 CPs, one per push, all build-verified).** Spec `~/Downloads/sidebar_mobile_mirror_spec.md`. Frontend-only except ONE additive db.ts helper (`fetchMyGroupJoinOrder` — viewer's joined_at per group in one query, for per-viewer "Group N" where full member lists aren't loaded). Desktop byte-stable throughout (shared-file changes are default-off opt-ins: `MobileSearchSheet.addLabel`/`groupContext.rejoin*` are mobile-only files anyway). Locked calls: **no TSP demo on mobile** (onboarding fires directly); **per-room leave lives in the show room's GEAR** (not an X on rows — no hover on touch, no accidental taps; adjusted from the click-sheet idea because in-room rows open the room directly).
> - **CP1 group-scoped proposals (`dbd1046`):** `MobileGroupRoom` vote/mode/opted-in now derive from the per-group vote (`gs.members` self `.voted`), never the personal pool; vote **no** = `setShowVote(false)` ONLY (the global `remove_show_from_pool` call is GONE from the group room); yes = vote + `ensureProgressRow` (S0E0, `in_pool:false`), optimistic `groupShows` patch + `refreshGroup`. In-group search PROPOSES: `MobileSearchSheet` gained `groupContext` (`groupShowIds` → "*[show]* is already in this group.", existing-progress hit → `proposeExisting` no-picker, else picker → vote + row); `addShow` mirrors desktop's (real progress writes globally, 0/0 stays out-of-pool).
> - **CP2 groups-only dashboard + four-part group room + naming (`0f04935`):** `MobileDashboard` REWRITTEN — personal shelves/show sheet/`removeShowFromPool`/`outOfPool` filtering deleted; page = groups rail + pending invites + centered **"Create another watch group?"**; sessionStorage snapshot key bumped (`ns_m_dash_snap2_`) so stale personal-layer snapshots can't paint. `MobileInviteSheet` REWRITTEN — create mode = the one-act (≥1 named friend, name+email both required per filled row, + ≥1 proposed show via in-sheet picker w/ chips + TVMaze; proposes on create; add-more mode keeps names optional + "send invite"); `onCreated` lands the user inside the new group. Group room = four parts (**OPEN SHOW ROOMS:** / **Proposed shows:** / "Propose more shows?" / "Add more friends to this group?" — add-friends moved from top to bottom). Naming everywhere: `groupDisplayName`/`groupGenericName`/`personDisplayName` + `fetchContactNames`/`fetchMyPendingInviteNames`; header title = per-viewer "Group N" (via `fetchMyGroupJoinOrder`), "with…" + avatar letters + interested-lines + "Read what X has written?" (@ dropped) = given names; gear gained the **"Update your contact list:"** card (first, desktop copy) + save via `setContactName`. `MobileGroupChat` bylines + connected-line = given names, no @.
> - **CP3 onboarding (`d767b13`):** new **`src/mobile/MobileSocialOnboarding.tsx`** — the 3-screen flow mobile-native (full-screen panels, centered): question over plain green → shared `MobileSearchSheet` (`addLabel="next"`) picks show+progress in one pass (picked-state renders an inline picker for the back-from-screen-2 case) → sky friend screen (name+email required, no skip) → the REAL `ComposeForm` (`mobileIdiom` + `privateOnly` + `externalSubmit` "publish & invite" + Header-1 override + Identity prompt button + ← back through `attemptDiscard`) → click-through confirm ("You're in."). Bootstrap = desktop's verbatim (group → started room → invite w/ name + auto-room → seed entry → **email LAST**, 2.5s retry / 401-403 → copy-link panel; idempotent via bootRef). Gate in `MobileDashboard` mirrors desktop: fires once on the dashboard for no-groups-no-invites accounts; invited accounts stamped unseen; pending-invite holders wait; `?sonb=1` force (no stamp); create-pill hidden while up. **No TSP demo on mobile** — mobile signups still don't set the demo flag (it fires on a first DESKTOP visit; unchanged).
> - **CP4 gated-entry stubs (`ca91a3f`):** `MobileShowRoom` builds stubs from `fetchGroupThreads().gatedThreads` + appends to the feed (`V2RoomFeed gatedStub` — already shared), passes `gatedStubAudience` (exactly one other current member → "you", else "the room"; departed don't count); stubs excluded from the visible-threads snapshot + `isNewMap` (white outline still fires on catch-up); stub-only room shows the feed, not "Be a trailblazer" (covers the CP3 invitee-behind-the-seed case).
> - **CP5 per-room leave (`b75e4ec` + copy fix in `0c173e0`):** the show room's digest GEAR gained "Leave this show room?" → left-justified confirm bottom sheet (desktop copy; "removes it from your list" per Alborz — people don't think in "shelves") → `leaveShowRoom(roomId)` → back to the group. `MobileGroupRoom` shelves hide `viewerLeft && !inRoom` (viewer-only; never-joined stay visible); in-group search lists a left room as "**· rejoin**" → `startShowRoom` re-join path clears the marker (`MobileSearchSheet groupContext.rejoinShowIds`/`onRejoin`). Also closes the CP1 leftover: desktop leaves now hide on mobile.
> - **CP6 solo → drafts (`0c173e0`):** private tab → **"drafts"**, desktop's reframed copy (draft freely / copy-paste into the friend room), `privateSubmitLabel="save draft"`. No solo entry point remains in `/m` (dashboard's died in CP2; `/m/show-room/private/:sid` = dormant deep-link only, desktop parity).
> - **CP-Naming (`3d4902b`):** `MobileShowRoom` fetches contact names + threads the `displayNames` map into BOTH `V2RoomFeed` instances (bylines, tooltips, stub names), the member-filter dropdown ("only [name]"), and the roster dropdown (names + avatar letters, @ dropped, "(you)"/"(left show)" kept); "with [group]" header now uses the desktop cluster rule (`groupDisplayName` incl. pending-invite names). `MobilePool` heading + empty-state = the VIEWER's name for the owner (anon keeps @handle). Ids/URLs/keys stay real handles everywhere.
> - **Verified during the arc:** Alborz ran the RPC audit (0 missing of ~54) + targeted checks (leave RPC, `viewer_left`, `inviter_display_name` column + accept seeding: all true). **Flagged, not fixed:** the pool-page proposal leak (open-arcs item 2 above).

> **2026-07-07 — Room-map rating cells: dice dots → STAR faces in CIRCLE cells.** Frontend-only; build-verified. New `src/components/v2/StarFace.tsx` (uniform coordinate model — cream 5-point stars, one 100×100 SVG; 1=center, 2=diagonal, 3=pyramid, 4=2×2, **5=regular pentagon, 6=pentagon+center**, per Alborz's grid reference) replaces `DiceFace` in `V2RoomMap` (`DiceFace.tsx` now dormant — no live importers; RatingCaptureModal has its own separate dice copy). `cellShapeStyle` radii CELL_RADIUS(8) → **50%** so every map cell is a circle (dead `CELL_RADIUS` const removed). Star fill is CREAM on every cell (a `StarFace` `color` prop exists, default cream — the brief outline-color-matched-star experiment was reverted per Alborz). Uniform star size across all ratings (`STAR_RADIUS = 0.17`); sizes/positions tunable at the top of StarFace. **Full-opacity pass (Alborz):** the outline cell states were washed out — dropped `opacity:0.7` (reached-no-entry) + `opacity:0.5` (not-reached) from `cellShapeStyle`, and swapped the non-self outline from the translucent `var(--dos-border)` (rgba .3) to the opaque greyblue `CANON.business`. KEPT (deliberate, not washout): multi-entry back-layer `opacity:0.3` (stack depth), member-filter dim `0.35`, and the spine-connector `0.55`. **NOT touched (flagged):** the RatingCaptureModal's pick-a-rating dice still shows dots — now inconsistent with the map; update to stars is a quick follow-up if wanted.

> **2026-07-07 — Proposed-shelf progress surfacing + delete-modal cancel restyle.** Frontend-only; build-verified. (1) A **Proposed** show button whose furthest opted-in OTHER member has real progress now shows a right-side label **"furthest progress: S# E#"** (highest across seasons via `linearIndex`; `furthestFriend` computed in the `groupShelves` memo, passed to `GroupPill`, takes the right slot ahead of the ▲/▼ gap; null → falls back to the gap/blank as before). Proposed-shelf opt-in **avatars now show per-friend tooltips** (`OptInAvatars withTooltip` flipped on for notStarted — the "(name) has watched: S# E#" / "hasn't started yet" tip already existed, was just disabled there; name = display/contact name). (2) `AccountModal` delete-confirm **Cancel** button → cream outline + cream text (was greyblue/dark), for contrast on the green card.

> **2026-07-07 — Group show-button FILL now = your relationship to the show.** Frontend-only (`groupPills.ts` computePill + `GroupPill`); build-verified. Replaces the old writer/watcher-count tiering + self-watching override — which measured a different axis than the CP1/CP2 shelf split and read confusingly. New rule (three states): **green** = there's an OPEN show room; **cream fill** (green text) = a PROPOSAL you've voted yes on; **cream outline, no fill** = a proposal by others you haven't opted into. On a proposal `members` holds only the yes-voters, so `self?.voted` = you're in. `selfWatching` is still computed (drives the ▲/▼ gap triangle + the click-modal's watchq resolution) — only its fill override is gone. Who-wrote is conveyed by the avatars + pen badges, not the fill. **Mobile note:** `MobileGroupRoom` shares `computePill`, so its fills shift to the new rule too (a touch early — mobile's still on the old opt-in model otherwise; accepted divergence, doesn't break).

> **2026-07-07 — CONTACT-NAMING ARC (3 CPs, one push): given-names EVERYWHERE, per-viewer "Group N", gear rename flow.** Alborz's phone-contacts model, fully applied. Frontend-only, NO migration (contact_names shipped in CP2); build-verified.
> - **CP-A — header restructure.** The group room header's TITLE is now the generic/custom label via new `groupGenericName()` — **"Group N" where N = the VIEWER's Nth group by their own join order** (computed client-side from memberships; per-viewer, so a rail can never hold two "Group 1"s — the per-creator `seq` is only the last-resort fallback and stops being displayed). The header's "with…" line switches from handles to **given names**. The SHOW room's "with …" now uses the cluster rule (given-names until custom name, incl. the viewer's pending-invite names) and the SAME style as the group room's members line (new shared `groupHeadingMembers` in dashboardChrome; greyblue "with", Inter 700 14 cream names). Cluster avatar letters = given-name first letter. Known accepted quirk: leaving a group renumbers your later groups.
> - **CP-B — the everywhere sweep.** One convention: an optional `displayNames` map (username → the viewer's given name; identity fallback) threaded `ShowRoomPage → V2RoomMap / V2RoomFeed → V2InlineThread → RepliesList → HighlightableBody` + `NudgePopover.recipientDisplayName` + `Username.displayName`. **Display-text only — ids, URLs (`/pool/:username`), avatar identities, and departed/filter KEYS all keep real handles.** Converted: map column headers + all cell/nudge tooltips, entry + reply bylines, deleted-entry/reply tombstones, ahead-of-progress reply stubs, CP4 gated-entry stubs, "responding to" labels, highlight hover attributions, the member-filter dropdown, chat ("You're connected with:" + message bylines via authorId), opted-in avatar letters/tooltips + "Read what … has written?" (via `memberNameById` now resolving display names), and the pool page heading (signed-in viewers see THEIR name for the owner; anon keeps @handle). **The "@" prefix is dropped wherever a display name renders** (phone-contact idiom; unnamed people show their bare handle). Received-invite prompts still show @inviter (naming-the-inviter deferred). Mobile + demo + dormant callers pass no map → byte-identical.
> - **Round-3 (Alborz):** the show-room header's "with" is now CREAM (inherits `groupHeadingMembers`, matching the names — was greyblue then briefly dark; the group-room "with" line still uses greyblue). **Groups-only dashboard now rests ~⅓ down** — the clusters + "Create another watch group?" were pinned to the top when sparse; the page is a flex column for the non-group context and its body (`dashboardCenter`, flex:1) holds the content between a 1:2 flex-spacer ratio (⅓ from top — split between top-pinned and dead-center per Alborz). Spacers shrink to 0 if content overflows, so the top stays reachable via page scroll. Group context untouched (page stays block flow; `clustersEl` extracted + rendered in both branches; edge tabs/sticky are position-fixed so DOM order is irrelevant).
> - **Round-2 (Alborz):** gear modal set now DIMS the page (same rgba backdrop as the site overlays); the contacts card moved FIRST with new copy — "Update your contact list:" + the phone-contacts explainer. **Migration `20260707_inviter_name_to_invitee_contacts.sql` (⚠️ APPLY):** accepting an invite now ALSO seeds the ACCEPTER's contact name for the inviter from the invite's "hi, it's…" name (`inviter_display_name`, written best-effort by the send-group-invite edge fn at email time) — the mirror carry; first name sticks; blank field → invitee sees the handle until they rename via the gear. NOTE: the onboarding flow has no "hi, it's…" field, so its invitees still start with the handle.
> - **CP-C — rename flow (gear).** The group gear gains a "Your names for the people here:" card — one input per OTHER member (placeholder = their handle), "save names" upserts via new `setContactName` (deliberate rename OVERWRITES, unlike accept-time seeding; empty clears back to the handle). Private to the viewer; every surface re-labels immediately (shared lookup, `contactNames` state refetched on save).


> **2026-07-06 — ONBOARDING LOOK-AND-FEEL PASS: the 3-screen flow now renders the site's REAL surfaces.** (The parked item from CP3, done right after the arc per Alborz.) Frontend-only; build-verified.
> - **The screens are the site's own overlays over the live dashboard** — no more special full-green pages, logo header, or position dots. Screen 1 = the REAL show-search card ("find your show", dash-result rows, TVMaze adds) → the REAL "How much have you watched?" picker card, with the onboarding question floating above in cream Header-1. Screen 2 = the REAL invite card (sky, cream fields, invitePill). Screen 3 = the REAL compose modal (ShowRoomPage's shell + the actual `ComposeForm` — ruled paper, header progress picker, posting… state, discard flow; "× not now" goes BACK to screen 2, no skip). Screen 4 = the site's accent confirm card. Copy-link fallback = accent card too.
> - **The spec's two deliberate differences are kept (Alborz: explicit differences are important):** the Header-1 line "Tell your friend why you're excited about **[show]**:" replaces the compose header's eyebrow+title (the picker beneath stays — which also satisfies "picker prefilled to the Screen-1 choice"), and the prompt button is Identity-fill "Want help with what to write?".
> - **New `src/components/dashboardChrome.ts`:** the modal styles (overlay/searchCard/pickerCard/searchInput/modalClose/yellowCard/yellowTitle/startBtn/invitePill/searchPill + ShowRoomPage's composeBackdrop/composeCardOuter) MOVED there byte-identical; DashboardPage + ShowRoomPage now import them (zero-visual) and SocialOnboarding reuses them — one source, no drift.
> - **`ComposeForm` opt-ins (default = byte-identical everywhere else):** `externalSubmit {label, onSubmit(draft)}` — the form validates + shows its posting… state but hands the draft (title/body/preview/tag/rewatch/insertedPromptIds) to the caller instead of publishing; `headingOverride`; `promptButton {label, background}`; `initialTitle`. The onboarding bootstrap now runs inside `externalSubmit` — same order, email still LAST with the retry/copy-link backstop.
> - **Plumbing change:** the declared progress is persisted at screen 2→3 (so the real ComposeForm, which reads the progress row, loads normally); the compose header's own picker persists any further change. Abandoning at screen 3 can leave that progress row (visible only on the mobile personal shelf — accepted edge).
> - **Round-2 QA (Alborz):** screen 1 opens with an accent **"Find your show"** button (the search card appears on click), the heading is TOP-ANCHORED (16vh) so result growth can't shove it, and the dashboard's "Create another watch group?" is hidden while onboarding is up (`socialOnbActive` gate — covers every step incl. confirm). The picker card's "pick a different show" → a top-left **← back** (like the invite card's). Screen 3: BOTH "not now" exits removed (new `ComposeForm.hideCancel` + `hideTopRightClose`), replaced by a top-left ← back routed through `attemptDiscard` (typed writing still gets the discard confirm; the prefilled title no longer counts as dirty — the check baselines against `initialTitle`). **Compose picker bug fixed FOR THE WHOLE SITE:** at zero progress the header picker DISPLAYED "S01 E01" (tag was correctly 0,0) — it now passes `allowZero` when the value is zero (same gate as ShowRoomPage's picker; pre-watch S0E0 writers were affected everywhere). Screen 4 = straight on opaque green (no modal): Header-1 "You're in." + Header-2 message + a create-group-sized "Get started!". The empty faded post pill until title+body exist is the regular modal's own behavior — kept.
> - **Round-3 QA (Alborz):** screen 1 no longer dims the page (transparent wrapper — the flow sits on the plain green); the confirm screen likewise lost its opaque cover AND is click-through (`pointerEvents:none` wrapper, `auto` content) so the logo + top-right account/sign-out stay visible and alive. Group room: shelf label → **"OPEN SHOW ROOMS:"**; "Add more friends to this group?" → Identity fill + cream text; both action buttons equal width (384) and moved further below the shelves (marginTop 56→80).

> **2026-07-06 — SOCIAL-ONBOARDING ARC CP1: GROUP-SCOPED PROPOSALS (the 07-03 one-state model is REVERSED).** Spec `~/Downloads/sidebar_social_onboarding_spec.md`; arc status + locked decisions in memory `[[project_social_onboarding_arc]]`. Desktop only (mobile mirrors in a later arc); build-verified.
> - **⚠️ MODEL: a not-started show now lives INSIDE a group as a PROPOSAL.** Proposing = the proposer's yes-vote (one action). The per-group `group_show_votes` rows are authoritative for not-started shows; `progress.in_pool` (the personal pool) no longer drives ANY group view. Watch progress stays global per show. This supersedes `[[project_optin_vote_model]]` (pool ⟺ yes-vote one state) — that memory now redirects here.
> - **Migration `20260706_group_scoped_proposals.sql` (⚠️ APPLY TO PROD):** (a) `get_group_dashboard` — a group's shows = votes-in-this-group ∪ rooms-in-this-group (a member appears on a show iff they voted here or are a current room member; progress read globally, all `in_pool` filters dropped; activity's progress leg scoped to engaged members). Shows that appeared in a group only because a member was WATCHING them (no vote, no room) no longer appear. (b) `start_show_room` — materialization on start = caller + current yes-voters only (the anyone-with-progress auto-enroll leg is dropped); vote rows preserved (a room supersedes its proposal, history kept). Room PERMANENCE already held (nothing soft-deletes rooms on votes/leaves). Frontend tolerates the old RPCs pre-migration (same payload shape).
> - **Shelves split by ROOM EXISTENCE (`groupPills.ts`):** room → "watching" (active-rooms shelf), no room → "notStarted" (Proposed shelf) regardless of voters' progress. Starting a room promotes the show off Proposed for everyone. Shared with mobile (`MobileGroupRoom` uses the same `computePill` + RPC, so mobile group SHELVES stay consistent).
> - **`DashboardPage` group context:** vote toggle + click-modal gating derive from YOUR yes-vote in THIS group (`gs.members` self `.voted`), never the personal pool. `doVote` yes = `set_show_vote` + quietly ensure an S0E0 progress row via new `ensureProgressRow` (db.ts — `in_pool:false`, `ignoreDuplicates`: proposals stay OFF the personal dashboard; existing rows never touched); optimistic self-vote patch on `groupShows` keeps the toggle instant. `doVote` no = withdraw the vote in this group ONLY (no `remove_show_from_pool`, nothing global); last yes withdrawn → proposal leaves the shelf. **In-group search = propose:** catalog hit with an existing progress row → `proposeExisting` (vote only, progress kept, no picker); new show → picker → vote + row (real progress picks write globally as always); "already added" message is group-scoped in group context. Personal dashboard side untouched (its × is still the global remove until CP5; personal shelves die in CP2).
> - **Interim quirks (accepted, die with CP2/CP5):** a proposal shows as "· restore" in the PERSONAL search (its row is out-of-pool by design); "just confirm my progress" no longer surfaces you on a group show (only a vote or room membership does); the invite-arrival screen (personal-pool-based) doesn't list proposals; a room whose members ALL left renders in no one's group view until someone re-adds the show. **Mobile divergence (accepted, next arc):** mobile sheet gating/vote actions still run the OLD in-pool model (a desktop-proposed show reads "no" on the mobile toggle; mobile vote-no still globally removes).

> **2026-07-06 — SOCIAL-ONBOARDING ARC CP6: SOLO JOURNALING → DRAFTS. ARC COMPLETE (CP1–CP6 all shipped same-day).** Frontend-only, no migration; build-verified.
> - **The show room's private tab is now "drafts"** (`ShowRoomPage` tab relabel + reframed in-tab copy: just-for-you, draft/journal freely, "copy and paste it into the friend room" when ready). Submit button on the drafts tab → **"save draft"** via a new `ComposeForm.privateSubmitLabel` prop (default "save privately" — mobile + the dormant standalone unchanged). Author-only visibility unchanged (the tab reads the viewer's own private threads).
> - **No convert-to-room control exists on the live surface** (verified — V1's convert/"Duplicate to…" lives only in dormant code) and **no desktop solo-writing entry point remains**: the personal-dashboard "write by yourself" modal died in CP2; `/show-room/private/:showId` (the standalone private-only room) now has NO desktop link — dormant deep-link only (mobile still has its own path; accepted divergence).
> - **Beta data flag (separate migration, NOT this arc):** the 5 beta users' old private-journal entries are intact and already read as drafts wherever the show has a reachable room (the drafts tab reads private threads per show); shows in no group are unreachable until the beta cutover migration. Nothing deleted.
> - **ARC WRAP — deferred/parked:** onboarding visual-fidelity pass (rebuild the 3 screens on the real compose/search/invite surfaces — Alborz flagged); the MOBILE mirror arc (per-group votes, groups-only mobile dashboard, drafts, per-room leave, stub audience); the beta-users cutover migration (personal shelves + private journals into the groups-only world, NOTHING deleted); haven't-started vs further-along prompt sets for onboarding screen 3; naming-the-inviter (received-end contact naming).

> **2026-07-06 — SOCIAL-ONBOARDING ARC CP5: LEAVE A SHOW ROOM (the per-room X).** Desktop only; build-verified.
> - **The X (hover-reveal, on SHOW ROOMS-shelf buttons you're a member of)** leaves ONLY that room in that group — never global (`remove_show_from_pool` untouched, still mobile-only). Confirm modal (accent card / cream title / outline buttons): "Leave this show room?" + the spec copy; [cancel][leave]. **Migration `20260706c_leave_show_room.sql` (⚠️ APPLY TO PROD):** new `leave_show_room(p_room_id)` — inserts the departed marker + deletes room membership; votes and the room itself untouched.
> - **The departed marker now does DOUBLE duty:** (a) the map's "has left" tag — but `get_room_map_data` only surfaces departed WRITERS (2026-06-28 rule), so a non-writer's leave is traceless everywhere; (b) per-viewer shelf hiding — `get_group_dashboard` gains `viewer_left` (caller has a marker on the show's room) and the client hides that button from THE LEAVER's view only. Deliberate-leave vs never-joined matters: never-joined rooms stay visible for discovery.
> - **Member display for ROOMED shows = current room members only** (proposals keep listing yes-voters) — a leaver's avatar drops off the button for everyone WITHOUT deleting their vote row (who-voted history preserved). Same rule flows to mobile (shared RPC/payload).
> - **Restore:** the in-group search lists a left room as "**[show] · rejoin**" — selecting re-enters via `start_show_room`'s re-join path, which clears the marker ("has left" clears on return; the transition already existed). Optimistic hide on leave; `refreshGroup` re-syncs.
> - Pre-migration: X calls a missing RPC (console error, no-op) and `viewer_left` reads false — nothing hides, nothing breaks.

> **2026-07-06 — SOCIAL-ONBOARDING ARC CP4: GATED-ENTRY STUBS.** Desktop only (mobile ignores the new data); frontend-only, no migration; build-verified.
> - **A spoiler-gated ENTRY now renders a one-line placeholder** in the show-room feed — the entry-level twin of RepliesList's ahead-of-progress reply stub (same `card redacted` shell, non-interactive): "**[friend]** has watched **S# E#** and written to you/the room. You can read this once you catch up." Progress shown = the episode the entry was WRITTEN at (matches reply stubs; locked answer). One stub per gated entry, sorted like a normal entry (by tag, then room-arrival time). Audience decided at DISPLAY time in `ShowRoomPage`: exactly one OTHER current room member → "you", 2+ → "the room" (departed members don't count). Covers CP3's ahead-case: an invitee behind the seed entry now sees the stub instead of an empty room (the stub count also means a stub-only room shows the feed, not "Be a trailblazer").
> - **Plumbing:** `fetchGroupThreads` now also returns `gatedThreads` (above-progress, non-deleted entries it previously dropped client-side; their shared_at recorded for ordering) — additive, mobile/dormant callers unaffected. `V2RoomFeedEntry.gatedStub` + `V2RoomFeed.gatedStubAudience` opt-ins (default off → all other feed callers byte-identical). Stubs are excluded from the room's visible-threads snapshot + `isNewMap` so the white "newly-visible" outline still fires when the viewer catches up to the real entry; deep-link/expansion/signals ignore stubs.

> **2026-07-06 — SOCIAL-ONBOARDING ARC CP3: 3-SCREEN ONBOARDING (show → friend → seed entry), invite email held until publish.** Desktop only; build-verified.
> - **Sequence:** signup → TSP demo (unchanged, still first) → NEW `SocialOnboarding` (full-screen, green) → dashboard. Gate = new `profiles.social_onboarded_at` (migration backfills ALL existing profiles → only post-migration signups see it; pre-migration frontend treats errors as SEEN — opposite default from the demo flag, deliberate). Fires on the BASE dashboard only, only when the account has NO groups and NO pending invites; an invited account (already in a group) is stamped WITHOUT seeing it; pending-invite-holders wait (re-evaluated next visit). Render is gated on `!showTspDemo` so the two can never stack. Test force: `?sonb=1` (no stamp). No skip/escape by design (spec).
> - **Screens (`src/components/SocialOnboarding.tsx`):** (1) "What's the show you're most excited to watch with a friend?" — catalog + TVMaze search, progress picker (allowZero), "(You can add more later.)"; (2) "Who's one friend you always text about TV?" — name + email BOTH required, "(You can invite more later.)", Invite advances only; (3) compose-style — Header-1 "Tell your friend why you're excited about **[show]**:", picker prefilled from screen 1, title prefilled "Let's do this!", Identity-fill "Want help with what to write?" (reuses `PromptCard`/`getPromptSuggestion`/`[PROMPT:]` token + `logThreadPrompt`); (4) confirmation → "Get started!" → lands in the new group.
> - **Bootstrap on publish (all idempotent across retries — created ids kept in a ref):** progress write → `createPeopleGroup` (UNNAMED — contact-name default names it after the friend) → `start_show_room` (room ALREADY STARTED → active shelf) → `createPeopleGroupInvite(gid, email, name, roomId)` (name = contact seed; roomId = auto-opt) → `insertThread` + `addThreadToGroup` (the seed entry) → **email LAST** via `sendGroupInviteEmail`. **Backstop:** transient failure → ONE auto-retry after 2.5s (animated `LoadingDots` throughout); 401/403 → no retry, straight to a copy-the-link panel (writing/room stay intact) → continue to screen 4. `sendGroupInviteEmail` now also returns `status`.
> - **Migration `20260706_social_onboarding.sql` (⚠️ APPLY TO PROD, after `20260706_contact_names.sql`):** `profiles.social_onboarded_at` (+backfill), `people_group_invitations.auto_room_id` (validated = live room of the group), `create_people_group_invitation` → 4-arg (older overloads dropped; 8-cap preserved), `accept_people_group_invitation` → also enrolls the accepter into `auto_room_id` (departed marker cleared) and returns it.
> - **Post-QA fixes (2026-07-06, Alborz):** (a) **accept lands on the DASHBOARD, not inside the room** — the land-in-room behavior was built then reversed on Alborz's call; the bootstrap room is auto-joined server-side so its button is waiting on the dashboard (db.ts still returns `roomId`, unused). (b) **Migration `20260706b_accept_ensures_progress.sql` (⚠️ APPLY):** accepting into a bootstrap room now also ensures a not-started S0E0 progress row (in_pool=false, existing rows untouched) — without one the room feed read NOTHING (the viewer's progress gates the read), so the S0E0 seed entry looked missing. An already-affected accepter self-heals by confirming progress once from the group's show button. (c) **PARKED for after the arc: onboarding visual-fidelity pass** — the 3 screens are standalone approximations; rebuild screen 3 on the real ComposeForm and screens 1–2 on the site's real search/invite surfaces once CP4–CP6 have settled the surfaces they'd reuse.
> - **Onboarding sticky:** the bootstrap group (localStorage `ns_onb_group`, set at publish) shows a CP3-specific `GroupRoomSticky onboarding` variant ("We went ahead and created a show room…") with its OWN dismissal key; all other groups keep the generic note. Subsequent groups via "Create another watch group?" stay proposals-only (no auto-room/seed/sticky) — already CP2's shape.
> - db.ts: `fetchSocialOnboarded`/`markSocialOnboarded`, `createPeopleGroupInvite` 4-arg with progressive fallback to the 3-/2-arg signatures pre-migration.

> **2026-07-06 — SOCIAL-ONBOARDING ARC CP2: GROUPS-ONLY DASHBOARD + FOUR-PART GROUP ROOM + CONTACT-NAME GROUP NAMING.** Desktop only; build-verified.
> - **Dashboard (green) is GROUPS-ONLY.** Personal shelves, the personal SEARCH, the personal show click-modal, and the personal × (global remove) are all GONE from desktop (`DashboardPage`) — the page is now: group clusters + one centered **"Create another watch group?"**. The top-right invite button was removed from BOTH contexts (its two roles moved into the bodies). Deleted dead code: `pillModal`, `removeConfirm`/`doRemoveFromPool`, `restoreShow`, `logProgressPersonal`, the personal shelves memo. ⚠️ `remove_show_from_pool` (RPC) now has NO desktop caller — the only exits from a room are the group gear's leave-group until CP5's per-room X; mobile still calls it.
> - **"Create another watch group?" = ONE act (modal):** ≥1 friend (name + email — BOTH required per filled row) AND ≥1 proposed show (in-modal picker: catalog + TVMaze add; chips with ×). Submit = create group → propose each show (vote + quiet S0E0 row) → mint invites (typed name rides each invite) → email legs with the existing copy-link fallback rows. "done"/×/click-out after a create lands the user INSIDE the new group (`?g=`). Validation gates the button ("create group"); the add-more-friends mode keeps label "send invite" and its name fields are OPTIONAL.
> - **Group room = four things (CP2 layout):** `SHOW ROOMS:` shelf (renamed from CURRENTLY WATCHING:) → **"Proposed shows:"** shelf (renamed from Haven't started yet:) → centered **"Propose more shows?"** (the search overlay — action already proposes since CP1) → centered **"Add more friends to this group?"** (cream, moved from the top-right corner). Chat tab, gear (rename/leave), and `GroupRoomSticky` retained. Search result copy: "already in this group"; a hit with an existing progress row proposes directly (no picker).
> - **Dual-mode group naming.** New `src/lib/groupNames.ts` `groupDisplayName()`: custom name (global) wins → else the names the VIEWER gave members (`contact_names`, handle fallback) + the names on the viewer's own pending invites → else "Group N". Wired: cluster labels + group heading (via new `GroupClusters` props) + `ShowRoomPage`'s "with [group]" header (same helper — fetches members + contact names when no custom name). **Migration `20260706_contact_names.sql` (⚠️ APPLY TO PROD):** `contact_names` table (owner-only RLS) + `people_group_invitations.invitee_display_name` + `create_people_group_invitation` gains optional `p_invitee_display_name` (old 2-arg overload DROPPED; 8-cap check from phase8 PRESERVED in both fns) + `accept_people_group_invitation` attaches the carried name as the inviter's contact name on accept (first name sticks — ON CONFLICT DO NOTHING). Frontend tolerant pre-migration (name param retries old signature; naming reads → {}).
> - **db.ts:** `fetchContactNames` (owner map), `fetchMyPendingInviteNames` (per-group names on my live invites; email local-part fallback; column-tolerant), `createPeopleGroupInvite(groupId, email, inviteeName?)`.
> - **Interim quirks:** with every desktop personal-pool surface gone, the 5 beta users' personal shelves are INVISIBLE (data fully intact) until the separate beta migration; mobile keeps its personal dashboard (divergence accepted). The received-invite cluster + invite emails still show inviter handle (naming-the-inviter deferred; Alborz: email handle is fine for now).

> **2026-07-03 — VOTE / OPT-IN MODEL (one state) + post-flip odds & ends. [MODEL REVERSED BY 2026-07-06 CP1 ABOVE]** All on `main`, build-verified (preview skipped per Sidebar rule). Commits `a4f7e48`, `9b396c8`, `741d9bb`, `2369b55`, `0c5da02`.
> - **⚠️ OPT-IN MODEL CLARIFIED (product rule, memory `[[project_optin_vote_model]]`):** for NOT-STARTED shows, being in your personal pool and being a "yes" in a group room are **ONE state**. Two doors in (add to dashboard / toggle yes in any group), two doors out (remove from dashboard / toggle no in any group) — either exit = **global un-opt** via the existing `remove_show_from_pool` (off dashboard, votes cleared in EVERY group, rooms exited; progress kept, restores on re-add). UI: the group click-modal (desktop `DashboardPage`) and show sheet (mobile `MobileGroupRoom`) now share one shape — **discovery-first**: vote question (+ trailer) leads; the progress picker + start/open-room sections render only when opted in, all active together. The toggle + gating derive from **in-pool** (`progress[id] && !outOfPool.has(id)`), NEVER the per-group `group_show_votes` row (a personally-added show has no vote row — reading it showed a false "no"). The old reduced vote-mode yes-state (room question without picker) is gone. Toggling no collapses the modal to the bare question. Client bug also fixed: mode resolution now treats out-of-pool shows as not-yours, so a removed show re-offers the vote question (the server had always cleared votes).
> - **Invite email failures now VISIBLE (both invite modals).** `sendGroupInviteEmail` returns `{ok, reason}` (detects non-2xx 401/429 AND Resend's silent 200-with-`email_send_failed` warning); desktop modal + `MobileInviteSheet` replace the false "Invites sent!" with "Sidebar is having an issue… copy the link… or log out, log back in…" + a copyable link row per failed address (desktop reuses the dormant `CopyRow`). **Root-cause note (memory `[[reference_silent_invite_email_401]]`):** "sent!" but no email = usually a STALE desktop JWT — PostgREST still mints the invite rows but the edge fn's `getUser` 401s; sign out/in fixes it. Triage: invocation 401 → token; 200 → check fn logs for "Resend error:"; 429 → rate limits (3 re-sends/invite/24h, 30 emails/user/day).
> - **ErrorBoundary auto-reload:** on a render crash the site reports to Sentry, then `location.reload()`s ONCE per 60s window (sessionStorage stamp = crash-loop guard) before showing the crash screen. Covers the stale-chunk-after-deploy class invisibly.
> - **Feedback tab on every live desktop surface** — `FeedbackWidget` now also renders on `DashboardPage` (dashboard + group), `ShowRoomPage`, and `PublicDashboardPage` (anon-capable, so the invite arrival has it too). Tab sits at 85% height; back tabs at 18% — no overlap. AppShell's instance unchanged.
> - **Map dice centering fix (`V2RoomMap`):** the hidden-entry (aboveViewer) cell is the one state with NO border, so its 32px content box left the 28px `DiceFace` ~4px up-and-left; cells now flex-center their dice (bordered cells were a snug 28-in-28 → zero visual shift elsewhere).
> - **Desktop homepage "Sign in"** → show-button-style pill (accent fill, cream text, rounded-65, Inter 700 14, 14/28 padding).
> - **Mobile panel rule (memory `[[feedback_mobile_panel_justification]]`):** full-screen panels CENTER elements; bottom sheets LEFT-justify. Applied: gear sheet (reverted to left), remove-from-pool confirm (buttons left: remove, cancel), pending-invite join prompt (left). **OPEN:** the show-room digest sheet is still center-aligned — Alborz deciding whether it conforms.
> - Mobile polish: write + remove-from-pool buttons centered (full-screen sheets), invite-arrival shelf headings all-cream, feedback sheet retitled **"Send Sidebar feedback."** + corner × + `pointerdown` tap-outside close (iOS ate the `click` that dismissed the keyboard).

> **2026-07-02 → 07-03 — MOBILE REBUILD ARC: `/m` rebuilt from scratch against the restructured world, then THE FLIP (phones now get the mobile app).** Spec `~/Downloads/sidebar_mobile_rebuild_spec.md`; checkpoint log + parked list in memory `[[project_mobile_rebuild]]`. Drill-down model: **dashboard → group room → show room → thread**; no tab bar; a shows↔chat toggle only inside a group. Desktop stayed byte-stable through the whole arc except two Alborz-approved changes (accent Sign in; the 07-03 odds above). Every push build-verified.
> - **Lockout lifecycle → THE FLIP (`76a911c`).** The arc ran behind a reinstated lockout (non-admins saw `MobileLockout` on every route; `/m/auth` exempt for the admin sign-in bootstrap; sticky `ns_admin_device` unlock for signed-out phone QA). **Flip:** the top-level App gate now redirects non-admin `<768px` visitors on any desktop-shaped path to `/m` (URL hash carried so Supabase email-confirm tokens survive the redirect). **Passthroughs:** `m`, `reset-password` (recovery links; the page is a narrow-friendly 420px card), and the param-preserving viewport forks `invite`, `show-room`, `group-invite`, `pool` — so digest `?entry=` and invite emails (which carry desktop URLs) deep-link into `/m`. Admins stay exempt (desktop QA from a phone). `MobileLockout` kept dormant as the rollback surface; rollback = revert `76a911c`.
> - **CP0 gut (`1a5fc4e`):** the 12 pre-restructure mobile screens (rooms/thread/respond/compose/edits/progress-gate/old invites/menu/search) DELETED (~4.9k lines). Salvaged: `MobileNarrative`, `MobileAuth`. `MobileApp` = thin router over `/m/*`.
> - **CP1 homepage parity (`d90dfeb`):** new **`src/lib/homepageCopy.ts`** = single source for the homepage WORDS (hero, 6 steps + icons, CTA labels, beta pill + letter); desktop `App.tsx` repointed (render-identical) and the mobile narrative rebuilt on it, killing the stale hand-copied duplicate. **Mobile-variant pattern** lives BESIDE the desktop constants so they rework together: `BETA_PILL_CLOSED_MOBILE` ("*tap here…"), `BETA_LETTER_PARAGRAPHS_MOBILE` (feedback-button paragraph).
> - **CP2 auth (`655dd5f`, `5132140`):** `MobileAuth` gained the full Forgot-password flow (mirrors AuthModal → `/reset-password`), `?mode=signup` entry + invite params (`?email` prefill, `?lock=1`, `?hint=`), a top-right ×, and 100dvh.
> - **CP3 dashboard (`f860672`, `f1d9f54`, `7b72550`):** groups rows (avatar strip + **ONE blue dot** for any new writing OR chat — spec amendment; the split shows inside the group), pending-invite join sheet, personal shelves with S/E, full-screen show sheet (progress / write-by-yourself / remove-from-pool), shared **`MobileSearchSheet`** (catalog + TVMaze + restore). **Instant paint:** page chrome renders during auth restore (was a blank screen) + a per-account sessionStorage snapshot hydrates the dashboard instantly on reopen (stale-while-revalidate; display-only, all writes stay live).
> - **CP4 group room (`28637dc`):** full-width two-line show rows (name / quiet gap line — the desktop hover-tooltip text moved inline; opt-in avatars + writer pen on the right; pill fill tiers), visible-only blue dots, shows↔chat toggle, full-screen click-model sheet (solo/vote/watchq + live "Read what … have written?" + **trailer below the actions** for a not-started viewer), gear rename/leave, in-group search, "Add more friends to this group?".
> - **CP5 chat (`cd809e8`):** full-screen realtime messaging (same member-gated `realtime.setAuth` note as desktop), linkified, mark-seen on open, auto-scroll; the group room gained the live chat-dot listener.
> - **CP6 show room (`3b295cb` + QA rounds `46e14d6`/`0eed322`/`661867a`/`aa2d2fb`):** REUSES the shared restructure components inside a mobile shell — `V2RoomFeed` (tickets, expansion, inline respond/edit, invisible-response stubs), `ComposeForm` (full-screen), `OneSelectProgress` + `RatingCaptureModal` (rating = the progress-advance capture ONLY). **Season map CUT → roster dropdown** (progress-ordered, raw S/E, "(you)" marked green/cream, departed "(left show)" = opaque accent avatar). Signals = the visible-writing subset (white new-since-visit outline / green response-on-your-entry / yellow highlight); the RED invisible-writing layer isn't computed at all. Digest gear bottom sheet; `?entry=` deep-links auto-expand cold with a real back-stack. **⚠️ New `mobileIdiom` prop (default false = desktop byte-identical)** threaded `V2RoomFeed → V2InlineThread → RepliesList` + on `ComposeForm`: mobile drops the Highlight…/Quote… creation buttons + ALL stars (Alborz may drop starring product-wide), cream-outlined reply Save, boxless edge-to-edge compose paper with in-flow action buttons. Shared `HighlightSpan` gained tap-outside dismissal (desktop no-op — mouseleave already closed it). Show-room toolbar = two fixed rows (order dropdown left + progress right, both width-capped so they can't push offscreen; write below).
> - **CP7 invites (`ef97d12`, `0f62ebb`):** `MobileInviteSheet` (dashboard "Invite new friends?" creates a group; group-room button targets it; desktop modal copy incl. "hi, it's…"), `MobileGroupInviteAccept` at `/m/group-invite/:token` (welcome pool shelves + JOIN IN → MobileAuth prefilled/locked, returnTo loop covers sign-in AND the email-confirmation link; join/decline + wrong/already/expired/invalid states with desktop copy).
> - **CP8 sweep (`ba42147`, `98ce7fc`, `413b289`, `43b7c3d`):** account gear (reuses `AccountModal` incl. delete-account; post-delete lands on "/" which the flip redirect now resolves to /m), **mobile feedback** = `MessageCircleWarning` dashboard top-bar button → `MobileFeedbackSheet` (same insertFeedback + cooldown as the widget), beta pill cream fills its outline, **accent "Sign in" on BOTH homepages** (the approved desktop visual change).
> - **Walkthrough (`6b6a598`, `771dc18`, `6ef40a8`):** mobile "Want more details?" = **`MobileHowItWorks`** — one vertical scroll (inline-numbered caption → static end-state diagram → narrow divider; Join CTA rests mid-screen at the end). Diagram data + captions EXPORTED from `HowItWorksV2` (export keywords only; the desktop pager is untouched); **`panelCaptionsMobile`** = scroll-tuned copy variant (trailing colons lead into the diagram below; "But in reality…" moved to slide 2's opener).
> - **Pool + stable back-swipe (`993191a`, `9f70dd6`):** `MobilePool` at `/m/pool/:username` (desktop pool copy/data, single column); show-room byline taps open it as an **OVERLAY on the still-mounted room with a pushed same-path history entry (`#pool=`)** — the iOS edge-swipe pops it and the room is exactly as left (expanded ticket, scroll, no refetch). Pattern reusable for future leaf pages; the GENERAL back-swipe-remounts case (room→group→dashboard) is parked until it bothers someone.
> - **Cut per spec:** season map, the entire ping/poll/SIKW system (both directions), map-based rating, red "newly-invisible" dots. **Parked:** remove-confirm restyle, TSP-tour-on-mobile decision (mobile signups currently get no tour and DON'T set the seen-flag → it fires on their first desktop visit), digest-sheet justification, hover-only tooltips in the reused feed components.

> **2026-07-01 (odds-and-ends batch) — pre-launch polish: digest gear, copy/privacy, delete-session fix, invite email.** Small iterate-on-live items, all shipped to `main` (build-verified; preview skipped per Sidebar rule). Colors added via canon (`[[canon_palette]]`).
> - **Show-room email-digest gear (reinstated).** Lucide `Settings` gear next to the show title in `ShowRoomPage` (**friend rooms only** — hidden in private-only `privateShowId`). Opens a "Leave this group?"-style modal (accent card, cream title, alert-outline button): subscribed → "Unsubscribe from email updates for this room?" / unsubscribed → "Resubscribe…". Wired to the EXISTING `get_room_digest_opt_out` / `set_room_digest_opt_out` RPCs + `friend_group_members.digest_opt_out` (shipped 2026-06-03; `send-digests` already honors it). Frontend-only, lazy fetch on open. `roomId` = the friend_group id (`p_group_id`). Commit `c487973`.
> - **Copy / privacy.** (a) Invalid-invite screen drops the raw token; shows an accurate message (invalid_token = the invite row is GONE — declined/withdrawn, inviter/group deleted, or a broken link; expired & already-accepted have their own screens). (b) Invite rate-limit message → "Too many invite requests sent…" via a new optional `limitMessage` arg on `checkRateLimit` (generic message unchanged for the ~10 other actions). (c) New `maskEmailEnds` util masks the email in BOTH "check your email" states (signup confirm + password recovery, desktop + mobile): `alkamalizad@yahoo.com → axxxxxxxxxd@yxxxo.com` (first+last kept, middle x'd). Existing asterisk-style `maskEmail` (room-creator pending-invites) untouched. Commits `0c6b1bf`. Widow fixes (nbsp via `preventLastWordOrphan`) on the invalid/wrong-email/rate-limit modal copy — commit `5b33c91`.
> - **Delete-account fixes.** Divider → cream. **BUG FIX (`74cf5ff`):** after account delete the page showed the anonymized `deleted_…` profile instead of the homepage — the anonymize model keeps a still-valid JWT (ban only blocks NEW logins), so if `signOut({scope:local})` didn't clear storage the hard reload restored it. Now force-removes any `sb-*-auth-token` from localStorage before `location.replace("/")`. (To clear a browser already stuck: run `Object.keys(localStorage).filter(k=>k.startsWith('sb-')).forEach(k=>localStorage.removeItem(k)); location.replace('/')` in the console.)
> - **Invite JOIN-IN button** (`PublicDashboardPage` `signInPill`) enlarged (14/40 → 20/72) + dashboard invite drop-shadow. Commit `6902c33`.
> - **Group show buttons** (`DashboardPage`): opt-in avatars/pen lowered 8px (`optInRow` translateY 50%+8) + `.group-pill-wrap` `margin-bottom:8px` (group-scoped) so they don't crowd the next row; s/e labels made fully opaque (dropped 0.85 / 0.8). Commit `5e762f2`.
> - **Invite email (`send-group-invite` + `send-invite`)** — commit `326f7a5`. Sender → `"Sidebar (no reply)"` (display name QUOTED so RFC-5322 doesn't drop the parens as a comment); footer "48 hours" → "a week"; legacy `send-invite` JS expiry 48h → 7d. New migration `20260701_invite_expiry_one_week.sql`: `people_group_invitations.expires_at` default 48h → 7 days (new invites only). New reference file `supabase/email-templates/confirm-signup.html` (styled like the invite email: indented 560px column, accent-yellow "Confirm your email →" pill, ✅). **⚠️ REQUIRES (done by Alborz):** deploy both edge fns; apply the migration; paste the confirm-signup HTML into Supabase Auth → Emails → Templates → Confirm signup; set the confirm-email SMTP **Sender name** → "Sidebar (no reply)".
> - **Homepage copy (`App.tsx`).** Rewrote the "Here's how it works" 6-step copy + icons (`UsersRound` · `ListChecks` · `Road` · `Eye` · `MessageSquareText` · `ShieldCheck`; old step icons DoorOpen/UserPlus/ClipboardList/Blend dropped) — commit `cba3d37`. Replaced the beta-tester letter (letters-from-the-future framing; note "feedback tab on the **left**" — now accurate, the FeedbackWidget tab is `left:0`; the old "right" was wrong). Then made the step cards consistent: dropped `textWrap:balance` (it re-balanced longer steps into narrow center columns) so text wraps to full card width (`c7b23d2`), and wrapped each step with `preventLastWordOrphan` to avoid last-word widows (`ac4e533`). Beta letter no longer mentions the TSP sample show, though the onboarding demo still auto-fires.

> **2026-07-01 (follow-up) — NO-WHITE RULE, TOKEN RENAME, + OPTION A: `canon.ts` is now the single lever for the WHOLE live site.** Continuation of the color arc below. All shipped to `main`, each zero-visual (values never changed — only where they're sourced from — verified by build; preview skipped per Sidebar rule).
> - **No pure white anywhere (blanket rule).** Every `#fff`/`#ffffff` → cream `#FEF8EA` (~582) and every `rgba/rgb(255,255,255,α)` → `rgba(253,248,236,α)` (127), site-wide incl. dormant. **ONLY exception:** the compose module's writing field in `ComposeForm.tsx` (ruled-paper input + box + textarea keep `#fff`). Legacy `#fffaf0` also → cream where LIVE (the `SidebarLogo` "white" block; dormant/dead `#fffaf0` left). Commits `b9b3d65`, `e828c88`. Standing rule in memory `[[feedback_no_pure_white]]`. ⚠️ Contrast tradeoff accepted (cream chrome on sky/personal frames is lower-contrast than white was).
> - **Canon token RENAME (intent-neutral).** So a token name never lies if its hue changes: `green→personal, sky→friend, blue→identity, yellow→accent, red→alert, midnight→dark, greyblue→business` (cream unchanged). Renamed the `CANON` keys, ALL `CANON.x` refs, and ALL `--canon-x` CSS var names (fallback hexes unchanged). Old hue names NO LONGER EXIST. Commit `05ebd5b`. Memory `[[canon_palette]]` updated with the mapping.
> - **Option A — component wiring COMPLETE (live + mobile).** Routed every remaining inline color literal (~450 across ~65 files) to canon: **JSX attributes → `{CANON.x}`** (works for lucide `color=`/SVG attrs — a hex string, unlike `var()` which fails in attributes), **standalone → `CANON.x`**, **compound CSS strings → `var(--canon-x,#hex)`**. `theme.ts` was already fully bridged (`147f78c`: all its canon hexes → `var(--canon-x,#hex)`). Batches: `18ad809` (b1), `dcb7101` (2a), `78b9cd8` (2b), `1af1f0f` (b3 v2+App), `a30fc96` (b4 mobile). **Final audit: 0 standalone/attr canon-hex literals left on any live/mobile surface.** Net result: editing one hex in `canon.ts` repaints the entire live site (theme.ts + every component).
> - **DORMANT/dead left as literals** (out of scope, deleted later): retired pages (V3JournalPage, ProfilePage, ShowSection, InlineThreadView, RepliesList, V2Profile*/V2FriendRoom/V2ComposePage, PublicProfilePage, Onboarding/ProfileThoughts*, ComposeModal, FriendProgressPostIt, InviteAcceptPage) + dead components (`SidebarLogoCanvasOverlap`, `YourShowsSelect`, `FriendRoomScroll`, `ModeToggle`). Transform helper: `scratchpad/canonize.pl`.
> - **STILL NOT wired (minor):** `canonContrast.ts` (never shipped — readability dropped); the CP5 Phase B `!important` public-room override collapse (dormant CSS — do WITH those components' deletion). `WHITE` constant in canon.ts is now `#FEF8EA` (side effect of the white sweep; it's an unused escape hatch).

> **2026-07-01 — COLOR STANDARDIZATION ARC (CP1–CP6): `canon.ts` is now the single source of truth for color.** From the Claude Design handoff (`~/Downloads/sidebar standardization.zip` → `design_handoff_canon_palette/`). Goal: exactly one place to change any color, on two axes — **AXIS-1** (edit a hex in `CANON` → every use changes) and **AXIS-2** (re-map a `ROLE` → only that usage changes). Scope = **Live + mobile, dormant excluded**. **NO readability/contrast work** (Alborz: "drop the readability concerns, adhere to the doc's colors") — `canonContrast.ts` was deliberately NOT shipped, and the white-on-green (2.21:1) fix was explicitly declined. Every checkpoint is **zero-visual** (verified via `npm run build` + hex fallbacks; preview skipped per Sidebar rule), each pushed to `main`.
> - **CP1** (`a655581`) — New `src/styles/canon.ts`: the 8 `CANON` colors, the `ROLE` semantic map, `withAlpha`, `frameGradient`, and the `injectCanonVars()` CSS-var bridge. Wired `injectCanonVars()` before `injectDOSStyles()` in `App.tsx`. Purely additive.
> - **CP2** (`5a0f051`) — Routed ~18 live/mobile components' local color consts to canon (local names + all usage sites unchanged; non-canon consts like `TEXT_MUTED`/`INK_SOFT` kept). Two approved drift-collapses folded in: HowItWorks `#375eb8`→canon blue `#355EB8`; FriendProgressPostIt `#fffaf0`→canon cream `#FEF8EA`.
> - **CP3** (`3e384fb`) — New `src/components/StickyNote.tsx` (shared cream/yellow sticky shell — paper/tilt/shadow/entrance/gate/dismiss-X, colors from canon, centralized `STICKY` constants). Migrated GroupRoomSticky + IncomingPingSticky onto it, pixel-faithful (−108 lines). **PollSticky/SIKWSticky left as-is** — two-state amber question widgets (no entrance fade, heavier shadow, custom dismiss); collapsing them would change visuals/behavior.
> - **CP4** (`7a18890`) — New `src/components/Button.tsx` (structural fill-vs-outline primitive from the draft). **Additive, no consumers** — it's opinionated (adds a hover wash + cream-over-white), so migrating call sites is a deliberate design pass, NOT a zero-visual step. Also routed 5 object-form `const C = {...}` palette blocks CP2's scan missed (ShowRoomPage, GroupInviteAcceptPage, PublicDashboardPage, AccountModal, TSPDemoModal) to canon.
> - **CP5a** (`deed560`) — Bridged `theme.ts` core vars to canon: the three room-context `--dos-*` blocks (`:root` green / `group-context` sky / `public-context` yellow) + all page-gradients now read `var(--role-*)`/`var(--canon-*)` **with the original hex as a fallback** (typo-proof; var names verified to match `canonCssVars()`). `theme.ts` and the TSX components now read the SAME `canon.ts` — no more two-systems drift.
> - **CP6** (`db95c23`) — Removed the unused Tailwind ghost colors (`ink/mist/pop/orange/gold`) from `tailwind.config.js` (verified zero utility-class usage; zero runtime impact) + deleted orphaned `Tabs.tsx` (no importers, not a rollback surface).
> - **DEFERRED / NOT DONE:** **CP5 Phase B** (collapse the ~11 `!important` public-room overrides into a `--room-accent` var) — investigated + declined: `body.public-context` and `.reply-card` are **DORMANT** (only `PublicProfilePage`/`ShowSection`/`ProfilePage`/`V3JournalPage`/`RepliesList`, all retired), so those rules are dead CSS outside the live scope; delete them **together with** those components when the rollback path is retired. **Button call-site migration** (design pass — accepts hover + cream-over-white). **Mobile** (`src/mobile/*`) uses inline hexes, not const blocks → a separate sweep if wanted. **`SidebarLogo` `BLOCKS`** array still holds inline hexes (visual-sensitive; not routed). **HowItWorks v1** (legacy `/how-it-works-v1` only) is dead-ish. See `[[canon_palette]]`.

> **2026-07-01 — PRE-LAUNCH SECURITY HARDENING ARC (all 10 checkpoints) + invite-flow rework.** Large multi-push hardening session ahead of opening to public testers; all shipped to `main` + deployed (migrations applied to prod + edge functions redeployed by Alborz). Origin: a security reviewer's 3 tips (IP/rate limiting, email verification, no frontend secrets) → expanded via 3 parallel audits (edge functions, frontend secrets, RLS) into a 10-checkpoint plan. Full context + the exact per-checkpoint state lives in memory `[[project_prelaunch_security_arc]]` and `[[project_invite_flow_rework]]`. Live restructured-surface *behavior* is unchanged except where noted — these are auth/security/privacy additions.
>
> - **CP1 — Email rate limits.** send-group-invite (3 re-sends/invite/24h) + send-message (poll_invite/sikw multicast + reply/request cooldowns) + shared 30-emails/user/day backstop. Commit `ce9a22e`. **⚠️ REPAIR `492b538`:** the checks were failing **OPEN** — calling `check_rate_limit` via a user-scoped client (anon key + JWT header) returns **"Auth session missing!"** in the Deno edge runtime, so limits weren't enforcing. Fixed with service-role `check_rate_limit_for` / `check_rate_limit_daily_for` (explicit `p_user_id`) — migration `20260630_rate_limit_for_service_role.sql` + admin-client calls. `send-invite` untouched (its 10/day cap is a direct `invitations` count query). **LESSON: the user-scoped-client → .rpc() → auth.uid() pattern does NOT work in edge functions; use an explicit-p_user_id SECURITY DEFINER fn granted to service_role.**
> - **CP2 — HTML-escape invite emails.** `send-invite` + `send-group-invite` now escape inviter/group names + the invite URL (send-message/send-digests already did). Commit `ce9a22e`.
> - **CP3 — Email verification.** `signUp` returns `{error, needsConfirmation}`; AuthModal + MobileAuth show a "check your email" state (enumeration-safe); `emailRedirectTo` routes general→/dashboard, invite→/group-invite/:token, mobile→/m. Commit `153cc56`. Dashboard side (Alborz): **Resend wired as custom SMTP** (was on the throttled built-in sender — the blocker), "Confirm email" enabled, redirect allowlist added, grandfather SQL (`UPDATE auth.users SET email_confirmed_at=now() WHERE email_confirmed_at IS NULL`). LIVE + tested. Testing gotcha: stale frontend bundle showed old behavior until hard-refresh.
> - **CP4 — Security headers on Vercel.** `vercel.json` now serves HSTS / X-Frame-Options / nosniff / Referrer-Policy / Permissions-Policy / X-XSS-Protection + a full **CSP** — these were ALL DEAD before (they lived in `netlify.toml`, which Vercel ignores). Rolled out Report-Only (`451a605`) → enforcing (`342069a`) after a clean browser-console pass. **CSP `connect-src` later extended with `https://*.sentry.io` (`4c1558f`) for CP9.** ⚠️ Adding any new external call requires adding its origin to the CSP or the browser silently blocks it — verified-origin map in `[[project_prelaunch_security_arc]]`.
> - **CP5 — Password strength.** 8-char client minimum on signup (AuthModal + MobileAuth) + reset (6→8); commit `1e2edaa`. Server-side min-length + leaked-password (HaveIBeenPwned) protection toggles confirmed set by Alborz.
> - **CP6 — Prod matches migrations.** Live SQL sweep in the Supabase editor: every `public` table has RLS enabled; all 52 frontend `.rpc()` names exist in `pg_proc`. 0 drift — the paper RLS audit confirmed on prod.
> - **CP7 — Self-serve account deletion (ANONYMIZE model) + bulletproof sign-out.** Commit `3cdeed1` (+ fixes `d3ac863`, `7b3c74a`). "Delete account" in the dashboard (⚙️ UserCog button by Sign-out) → type-DELETE confirm → `delete-account` edge fn → `anonymize_own_account(p_user_id)` RPC (migration `20260630_anonymize_own_account.sql`; **service_role-only, explicit p_user_id — same "Auth session missing!" fix**) then scrubs `auth.users` email/password + 100y ban via Admin API. Behavior: PII + PRIVATE writing hard-deleted; SHARED-room entries KEPT as "[deleted]" with `author_id` NULLed so the person drops off map/members/chat (feed reads the `author_name` snapshot); groups they CREATED stay intact for other members (auth.users KEPT-not-deleted to dodge the created-groups/group_messages CASCADE). Private-thread delete guarded with `is_public=false` so it can't take others' replies on a public post. Post-delete → local signout + hard-reload to homepage. **`signOut` (auth.tsx) now nulls user/profile state SYNCHRONOUSLY before network calls** so a dead/banned/dangling token can't trap the UI on an authed screen. Verified end-to-end on throwaway accounts.
> - **CP8 — Privacy notice at signup.** Inline §16 Body-font (Inter 13, cream) line in signup mode on AuthModal + MobileAuth explaining email use (never shared/sold). Commit `83bfe8b`. Copy is placeholder — rework pending.
> - **CP9 — Remote error tracking (Sentry).** `@sentry/react` v10; `src/lib/sentry.ts` `initSentry()` (index.tsx) + `captureError()` forwarded from `ErrorBoundary.componentDidCatch`. Gated on `VITE_SENTRY_DSN` — with no DSN the SDK **tree-shakes out entirely** (zero bundle cost); DSN set in Vercel → active. Commit `173a316` (+ CSP allowance `4c1558f`). Verified (test error received). Errors-only; global handlers cover async/event-handler errors the boundary misses.
> - **CP10a — Shows-catalog column lock.** `shows_update` was `USING(true)` = any authed user could edit any show (rename/image/hide) via direct API. Now a **column-level GRANT** limits authenticated UPDATE to the episode-refresh columns (`seasons/last_synced_at/genres/tvmaze_type/status`); name/is_hidden/etc are admin-only. Admin hide-toggle moved to `admin_set_show_hidden()` (SECURITY DEFINER + is_admin()). Migration `20260630_lock_shows_catalog_columns.sql`, commit `7390861`. `refreshShowIfStale` / add-show / admin delete unaffected.
>
> **Invite-flow rework** (`[[project_invite_flow_rework]]`):
> - **Inviter display-name field** (commit `45da7aa`, migration `20260701_invite_inviter_display_name.sql`). New "hi, it's…" field + §16 Body-font/cream explainer in the invite compose modal (DashboardPage). Name stored in new `people_group_invitations.inviter_display_name` (written best-effort by `send-group-invite`), returned by `get_people_group_invitation`. Shown as **"Johnny (@handle)"** in the email **subject** + welcome-screen auth line; email **body** + arrival headings keep bare `@handle`; falls back to `@handle` when blank. **CORRECTION to a prior assumption:** the welcome screen never showed the inviter's *email* (always `@username`) — the real deferred CP10b concern is the INVITEE's email + inviter watchlist in the token payload.
> - **Decline + accept fixes** (commit `319790e`). Welcome-screen **"no" now DECLINES** (deletes the invite via `declinePeopleGroupInvite`) instead of leaving a lingering red pending cluster — invitee lands on a clean dashboard; to reconnect they send their own invite. Dashboard pending-invite **"yes did nothing" bug fixed**: root cause was an unguarded `loadRail()` inside `refreshRailAndInvites` that threw AFTER a successful `accept_people_group_invitation`, so the `navigate(?g=)` never ran (join succeeded server-side, UI never moved). refreshRailAndInvites now fully try/caught; genuine accept failures surface a reason in the prompt. Direct-login pending-accept path verified working.
>
> **DEFERRED (non-blocking, all in memory):** CP10b invite-token payload trim (bundle with the next invite-flow work); privacy-copy rework; email deliverability is already solid (SPF/DKIM/DMARC all set — verified via dig; the one spam hit was a repeated-test artifact) with optional future `p=none`→`p=quarantine` DMARC tightening.

> **2026-06-30 — Trailers SHIPPED, departed-map writing-gate (migration), dashboard perf, group-button indicator overhaul, departed-dot polish, logo home button.** Big iterate-on-live session; all live restructured surfaces, mobile + dormant code untouched.
>
> - **TRAILERS — BUILT & LIVE (was the parked NEXT-UP).** YouTube launch-trailer embed beneath the group opt-in modal, shown ONLY to a not-started viewer (gate `curVal.s===0 && curVal.e===0` on **persisted** progress — covers vote / watchq / solo-at-S0E0; never tied to the draggable dropdown). Frontend-only, **zero Supabase egress**. New `src/lib/trailers.ts` (external-id bridge `show.tvmazeId`→TVMaze `externals.imdb`/`thetvdb`→TMDB `/find`→`/videos`, earliest official YouTube **Trailer** w/ **Teaser** fallback; in-memory hit+miss cache keyed per showId; tolerant→clean miss on any error) + `fetchTvmazeExternals` in `tvmaze.ts` + `src/components/TrailerCard.tsx` (facade `i.ytimg.com/.../hqdefault.jpg` thumbnail → click swaps to a `youtube-nocookie` iframe; renders **nothing** until a hit; width `min(460px,92vw)`; **TMDB attribution line at the bottom — required by the free Personal-use license**). Wired into `DashboardPage`'s clicked-modal inside a **dedicated scrollable two-layer overlay** that centers modal+card as a pair (8px gap; the shared `overlay` style is untouched). **Prefetch (#1):** `prefetchTrailers()` warms the cache for the active group's not-started shows, deferred via `requestIdleCallback` AFTER the group page paints (never blocks load; bounded concurrency; dedupes). **ENV — DONE:** `VITE_TMDB_READ_TOKEN` (TMDB **v4 read token**) is set in **Vercel (Production)** + local `.env.local`; absent → silent miss everywhere. Bridge verified live (Breaking Bad, GoT — correctly picks the earliest launch trailer, not a season-N spoiler). **Scope:** group opt-in modal only (personal-dashboard not-started modal deferred). **License:** TMDB Personal-use tier (non-commercial); switch to a commercial license via TMDB only if the site ever monetizes. Commits `af5598e`, `f832873`. Spec `~/Downloads/sidebar_trailers_spec.md`; `[[project_trailers_spec]]`.
>
> - **Departed-member map column now requires WRITING in that room. ⚠️ MIGRATION APPLIED TO PROD.** A user who X's a show out of their dashboard keeps a map column (with the terminal dot) **only if** they authored ≥1 non-deleted **entry OR reply** in THAT room; ratings/progress alone no longer keep a column. Per-room, objective (viewer-independent), retroactive (read-time filter). Migration **`20260628_room_map_departed_requires_writing.sql` — APPLIED to prod by Alborz** — adds a writing-existence filter to the departed branch of `get_room_map_data`'s `member_set` (same return shape/authz; idempotent CREATE OR REPLACE). Frontend unchanged (`ShowRoomPage` just renders the columns the RPC returns). Commit `2120b04`. `[[project_departed_writer_shelf_followup]]`.
>
> - **Dashboard perf — secondary loads parallelized.** The groups rail / pending invites / new-activity dots loaded in a sequential waterfall after the core data; now they fire **concurrently** (each sets its own state, independently tolerant), cutting two round-trip waits off time-to-fully-loaded. First-paint `Promise.all` (shows+progress+out-of-pool) unchanged; shows-catalog cache + non-blocking episode/trailer prefetches already good. Commit `24dd23d`. **OPEN (Batch 2, if still slow):** the per-group rail fan-out (`loadRail` → ~3 round-trips per group) is the next lever but needs a batched RPC — **measure first** (rail fan-out vs. a single heavy aggregation RPC).
>
> - **Group show-button indicators — overhaul (final state).** Touched `DashboardPage.tsx` + `groupPills.ts`. (a) **Sole-writer** dropped the green avatar fill; (b) the writer mark is the **bare Lucide pen icon** (cream strokes, **sky fill**, NO circle/dot behind it), at top/right `-5` on the avatar; (c) shown on **every** writer's avatar — removed the lone-writer `markWriter` gating AND the 2+-writers `UsersRound` people icon entirely; (d) **self-watching show buttons** (you've started the show) render **green fill + green outline** via a new `selfWatching` flag on `PillData`; (e) **gap indicators** are now **Lucide `Triangle`** (not Unicode glyphs): **up = cream solid fill, down = red solid fill, no outline** (rotated 180° for down), count in **cream**. Commits `bcaeab3`, `0e39af8`, `4374351`, `5b4c7ec` (+ superseded iterations `592e0e3`/`d97f987`/`33696f7`).
>
> - **Departed terminal dot (`V2RoomMap.tsx`) — alignment + color polish.** The "left the room" dot drifted right + read as detached (its Tooltip wrapper wasn't anchored). Anchored the tooltip so the dot **centers on the departed member's column**, added a short spine connector to the last watched cell, sized to **8px**, dropped it 4px, and recolored to solid **canon greyblue `#8DAABA`** (also swapped the map's hidden-entry cell fill `#819dad`→`#8DAABA`). Tooltip hit-area aligned to the dot. Commits `995423c`, `5b1ee93`, `bef9494`, `16fb831`.
>
> - **Dynamic logo is now a HOME button** (`DashboardPage`, `ShowRoomPage`) — clicking the in-app `SidebarLogo` navigates to `/dashboard` (clearing any group context). Other logo instances (homepage / onboarding / error / invite / mobile) left non-navigating. Commit `980b856`.
>
> - **Canon palette / memory:** recorded the authoritative site palette in `[[canon_palette]]` — green `#7ABD8E`, sky `#ADC8D7`, blue `#355EB8`, yellow `#DEA838`, red `#F45028`, **cream `#FEF8EA`** (NEW canonical value; standing rule "when Alborz says *cream*, use `#FEF8EA`"), midnightblue `#1A3A4A`, **greyblue `#8DAABA`** (NEW). ⚠️ The codebase still has legacy `#fffaf0` cream in many places — NOT mass-migrated; only change in scope.

> **2026-06-28 — Stale episode lists fixed: live surfaces now refresh shows from TVMaze again.** No migrations. **Bug:** since the 2026-06-24 cutover, NO live surface refreshed a show's season/episode counts — `refreshShowIfStale` (db.ts, 12h cadence) was only ever called from the now-retired `ShowSection`/`ProfilePage`/`V3JournalPage`. The live dashboard + show room just read the cached `seasons` array, so any show that aired new episodes after it was first added was frozen (surfaced via "The Bear" — its newer seasons never appeared in the progress dropdowns). **Every** progress dropdown across all three surfaces (personal dashboard, group room, show room) reads `seasons` from the SAME shared catalog (`showsById` in DashboardPage / `show` state in ShowRoomPage; `GroupDashboardShow` carries no seasons of its own) — so one refresh path fixes all three. **Fix (frontend-only):** new `refreshStaleShows(shows[])` helper in `db.ts` (bounded concurrency 4 to spare TVMaze's rate limit; returns only the shows that actually changed; never throws). Wired non-blocking into the load step of: (1) **dashboard core load** → refreshes the viewer's pooled shows (`prog` keys), merges results into `shows` state; (2) **`refreshGroup`** → refreshes the active group's shows (incl. ones you haven't pooled), same merge; (3) **`ShowRoomPage.load`** (both the room + private-only branches) → new `freshenShow()` calls `refreshShowIfStale` on the one show, guarded so a late result for a different room is ignored. Cadence gate makes ~every call a cheap in-memory date check; only genuinely-stale shows hit TVMaze (free/external → **zero Supabase egress**); a real change writes one row → realtime patches the catalog cache. Untouched: the 12h cadence, mobile, dormant code, the entire-catalog (we only refresh shows the viewer can act on). Touched: `db.ts`, `DashboardPage.tsx`, `ShowRoomPage.tsx`. **Caveat:** a show with no stored `tvmazeId` still can't refresh (none exists) — first loads will reveal any such shows. **PARKED follow-up (deliberately not built):** an admin "refresh episodes" button in `AdminPage` to force-resync one (or all) shows on demand as a backstop — would just call `refreshShowIfStale`/`refreshStaleShows`; come back to it if needed.

> **2026-06-26 — Group/show-room polish batch + the "Read what … have written?" prompt (with migration).** All live restructured surfaces; mobile + dormant code untouched.
> - **"Read what … have written?" group click-modal prompt (NEW, migration applied).** In the **solo** + **watchq** click-modals, when a room exists and ≥1 OTHER member has an ENTRY **visible at the viewer's LIVE in-modal progress**, the prompt becomes **"Read what your friends have written?"** (≥2 visible writers) / **"Read what @user has written?"** (exactly 1) and the button reads **"Read"**; otherwise the existing "Open show room?" / "Start a show room?" + "Yes". Updates **live** as the progress dropdown moves; "Read" and "just confirm my progress" both save the changed progress, X/click-out doesn't (unchanged). **Migration `20260626_group_dashboard_entry_min_episode.sql` (APPLIED to prod by Alborz)** adds per-member earliest-written-ENTRY episode (`wrote_entry_min_s/e`, entries-only) to `get_group_dashboard`; additive, pre-migration servers return null → prompt falls back to Open/Start. Entries-only is sufficient (an invisible entry can't have a visible reply). **Pure-visibility by design:** a visible pre-watch S0E0 note shows "Read…" even at S0E0 (accepted — there's genuinely something to read). Touched: `db.ts` (`GroupDashboardMember.wroteEntryMinS/E`), `DashboardPage.tsx` (visible-writer calc via `linearIndex` on `declaredProgress`). Commit `5095a7e`.
> - **Show room header — "[Show] with [group name]" + taller bar.** The `/show-room` header now appends **"with [group name]"** in blue Header-1 (Lora bold 34) after the show name (group display name = custom else "Group N", via `fetchPeopleGroupsForUser`; private-only standalone shows just the show name). Header bar height **92 → 104px** (`HEADER_H`, 8px grid) — single header element so it applies to BOTH tabs (green friend / sky private). Both shipped together in commit `ea285ad` (note: that commit's message only mentions the height; the group-name feature rode along — both are live). Touched: `ShowRoomPage.tsx`.
> - **Edit spoiler-gate retag warning — restyled as a visible modal.** The gate LOGIC was never lost; the warning rendered as an inline banner with `background: var(--dos-bg)` = sky in group-context = the same color as the show-room background, so it blended in and read as "missing." Converted to a centered `Modal` (cream card, Header 2 + Body, midnightblue, pill buttons). Copy unchanged ("Heads up — this post will be retagged" / "Save & retag" / "Go back"); gate + tag-bump-on-save unchanged. `V2InlineThread.tsx`. Commit `0cb9119`. (Dormant V1 `InlineThreadView` left as-is.)
> - **Group chat: URLs auto-linkify** (`DashboardPage.tsx`) via the shared `linkifyText` helper (new tab, `rel="noopener noreferrer ugc"`, wraps) so members can share trailer links etc. Display-only. Commit `91d61e7`.
> - **Edge tabs (continued):** after the ~50% enlargement, the chat **notification dot** drifted off into the rounded cutaway; repositioned onto the upper portion of the tab's rounded left edge (`top:10,left:0`). Commits `487ff9b`, `87d75b7`.

> **2026-06-25 (follow-up) — Invite-screen + group-room polish, the in_pool/gap-indicator bug, opt-in gating, tab sizing, and a dev wipe tool.** Continuation of the same day; all live restructured surfaces; mobile + dormant code untouched. No migrations.
> - **Gap-indicator bug (root-caused + fixed).** An invitee saw a red ▼ progress gap on EVERY show another member pooled — and a member's gap wouldn't update for some shows. Root cause: `get_group_dashboard` only counts a member's progress when their `progress` row is `in_pool = true`, and the main progress-write path (`persistProgressUpdate` → `upsertRewatchStatus`) never set `in_pool`, so a row stuck at `in_pool=false` (e.g. from the BTU cutover, which set BTU users' Paradise to out-of-pool) was ignored forever. **Fix 1 (`db.ts`):** `upsertRewatchStatus` now sets `in_pool=true` on every progress write — writing/updating progress re-opts the show into your pool (like voting "yes"); remove-from-pool stays the deliberate opt-out. Commit `888125a`. **Fix 2 (`groupPills.ts`, Option B, frontend-only):** the not-watching ▼ renders ONLY once the viewer has opted into that show — opted-in = watching (progress past s0 e0) OR wrote in its room (any author). A show another member pooled that you haven't engaged with shows a blank right side. Commit `89cc1ee`. **Fix 3 (`DashboardPage.tsx`):** the show-button tooltip (progress/gap/notif) is likewise suppressed on non-opted-in shows; the opt-in avatars keep their own per-user progress tooltips. Commit `6043afd`.
> - **Dashboard invite buttons reworked (`DashboardPage.tsx`).** The old top-right "INVITE FRIENDS" + the centered "CONNECT MORE FRIENDS TO THIS GROUP" became ONE context-aware top-right button (the centered button + its style deleted). Final state: dashboard → **"Invite new friends?"** (blue, white text, keeps drop-shadow); group room → **"Add more friends to this group?"** (cream fill, green text, NO drop-shadow). Same `invitePill` base. Also: group-room content column (heading/shelves/show buttons) lowered 16px (banner/clusters/tabs unchanged). Commits `11530f4`,`09e747f`,`345faaf`,`6d0767a`,`ab4da5a`.
> - **Invite arrival screen (`PublicDashboardPage.tsx`).** Shelf headings centered (were left-aligned); all heading text cream (the `@username` was the lone white bit); watching heading adapts — "and is already watching these:" when a want-to-watch shelf precedes it, else standalone "@X is watching these shows:". Commit `8832226`.
> - **Invite modal + sticky copy (`DashboardPage.tsx`, `GroupRoomSticky.tsx`).** "Invites sent!" modal: "done" button centered. Email-invite header: "Email a friend to..." → **"Email friends to start a watch group:"**. Group-room sticky: "each others'" → "each other's". Commit `2655db9`.
> - **TSP demo (`TSPDemoModal.tsx`).** Guide posts now open EXPANDED the first time each episode is reached (still collapsible; single-expansion closes the prior; ep0 still via `initialExpandedThreadId`). Commit `7725231`.
> - **Edge tabs enlarged ~50%, on the 8px grid (`DashboardPage.tsx` back+chat, `ShowRoomPage.tsx` back).** padding 32/40/32/24, radius 48 (all multiples of 8 per spec §16); icons unchanged. Commits `7f3a87f`,`91c4207`. **(Reminder honored: §16 spacing = multiples of 8, tight exceptions 4 — see `[[canon_typography]]`.)**
> - **Dev tool (NOT in repo):** built a safe single-user wipe SQL block (resolve by email → protected-account guard for BTU/seed → FK-safe deletes of likes/replies/threads/progress/group_messages → delete profile + auth.users, all in one transaction) so Alborz can re-test the invite flow with the same email. Pressure-tested: the only real blast-radius is that deleting a user CASCADE-deletes groups/rooms they CREATED (people_groups.created_by + friend_groups.created_by/parent_group_id are ON DELETE CASCADE) — groups they only JOINED survive. Alborz accepted that (test rooms are throwaway). **Note for invite testing:** the group-room sticky's "dismissed" state is browser `localStorage` (`ns_group_room_sticky_dismissed`), NOT per-account — a DB wipe won't bring it back in the same browser; use incognito/clear the key.
>

> **2026-06-25 — Invite sign-in for existing accounts; group-room sticky + empty-state polish; copy fixes; rating-modal restyle.** Iterate-on-live session, all on the live restructured surfaces; mobile + dormant code untouched.
> - **Existing-account invitees can now sign in from the JOIN IN screen (no dead-end).** Previously a signed-out person with an account who hit JOIN IN was pushed into account *creation* (silent dead-end). Migration **`20260624_restructure_invite_has_account.sql` (APPLIED to prod by Alborz)** adds `invitee_has_account` to `get_people_group_invitation` (token-scoped `auth.users` existence check — not email enumeration). `GroupInviteAcceptPage.tsx` opens `AuthModal` in **signin** mode when the invited email already has an account, and after a sign-in shows the existing on-page "Join a group with @X? Yes/no" confirm instead of bouncing. `AuthModal.onSuccess(mode)` now reports signin-vs-signup. `App.tsx` suppresses its generic sign-in→`/dashboard` redirect **only on `/group-invite`** so the page owns its post-auth routing (brand-new invitee signups still route via `ns_post_signup_dest`, unchanged). Touched: `db.ts` (`GroupInviteInfo.inviteeHasAccount`), `AuthModal.tsx`, `GroupInviteAcceptPage.tsx`, `App.tsx`. Commit `7fb306b`. **(Verified the four BTU members ARE existing group members — so the no-code "sign in normally → dashboard prompt" path does NOT work for them, because the dashboard hides pending invites for current members; this on-page sign-in flow is the route that gives them the full experience. Accept is idempotent for current members.)**
> - **Group room (dashboard group context) — explainer sticky + empty-state polish.** New `GroupRoomSticky.tsx`: dismissible cream tilted note (canon sticky look) explaining shows accumulate / selections are shared / where chat lives / "the chat box isn't spoiler-gated"; dismiss persists per-browser (`localStorage ns_group_room_sticky_dismissed`, no DB), hidden <1160px, positioned vertically centered in the right gutter. Mounted in `DashboardPage` group context only. **Empty-state prompt** ("What shows are you watching or thinking about starting?") pulled up to sit just below the group clusters in BOTH the no-group home state (`heroWrap` top-aligned, was centered) and the group-context empty state (which gained the same heading above the SEARCH button). Invite welcome heading **"Want to watch something with them?" → cream** (`PublicDashboardPage`, was white). Commits `8b07765` + sticky nudges (`dd302b6`,`c5482be`,`278c68d`).
> - **Copy fixes.** The lone-opted-in dashboard modal "Start a solo show room?" → two lines **"Start a show room?" / "Your friends can join in when they're ready."** (`DashboardPage.tsx`; never suggest a "solo" room — the other two cases, open-existing / friends-already-opted-in, unchanged; the SearchShows "watching by yourself" private-writing flow left alone). **TSP demo guide entries 1–4 reworded** (`tspDemoFixture.ts`): lowercase "episode", trailing "advance to the next episode" nudges, "The map keeps score" title, an added "click the check mark to confirm" step in the map-rating instruction; collapsed-card previews synced. Entries 0/5/6 untouched. Commits `148cd7b`, `8c77833`.
> - **Rating-capture modal restyle (`RatingCaptureModal.tsx`).** Cream pills (65px radius), **solid sky dice with cream dots** drawn locally (4px square radius; the map's white-dot `DiceFace` is left untouched), rating labels lose trailing periods. Fonts now per **restructure spec §16** (see new memory): heading = Header 2 (Inter bold 14), rating + cancel + skip = Body (Inter regular 13), **normal letter-spacing** (the spec's original −2 was dropped by Alborz 2026-06-25). **Added "skip rating"** (cream 2px outline, like cancel) = confirm the progress advance but write no rating; new optional `onSkip` wired into the live callers (`ShowRoomPage`, `ComposeForm`); dormant `V3JournalPage`/`V2FriendRoomPage` get the restyle but no skip button. Commits `2070f3b`, `51c3772`.
> - **Cancelled mid-session:** adding the group chat button/box to the show room (built then fully reverted at Alborz's request — `ShowRoomPage` clean, no `GroupChat.tsx`). "No more changes to dormant pages."
> - **New memory `canon_typography`:** §16 of `~/Downloads/sidebar_restructure_spec.md` is the authoritative type spec — Header 1 Lora bold 34 / Header 2 Inter bold 14 / Body Inter regular 13 / Info Inter regular 10, **normal letter-spacing**. ⚠️ `theme.ts --t1..--t4` (22/17/15/13) is an unrelated OLD scale that does NOT match §16 — don't infer font sizes from it. (Got this wrong twice this session.)

> **2026-06-24 (follow-up) — Post-cutover polish + the "applied-but-not-really" migration sweep.** A long iterate-on-live session after the cutover. All on the live restructured surfaces; mobile + dormant old code untouched.
> - **⚠️ Migrations logged as "applied" that WEREN'T on prod** (each only surfaces when a user first hits that path) — now applied: **`get_group_pending_invites`** (phase9 `20260620_restructure_phase9_group_avatar_states.sql`; pending-invitee cluster avatars came back empty without it), **`lazy_close_room_polls`** (`20260506_polls_phase_2e_lazy_close.sql`; expired polls weren't auto-closing — caller swallows the error so it was silent). Also applied: **`20260624_restructure_decline_group_invite.sql`** (decline-invite RPC). **Repo↔prod drift fixed:** committed the polls **2a/2b** migrations (live on prod, never tracked). **Audit tool saved:** `[[reference_rpc_audit]]` — one-shot query lists frontend RPCs missing from `pg_proc`; result is now 0. **Don't trust HANDOFF "applied" claims — verify.**
> - **Email invites NOW WORK:** `send-group-invite` edge fn **was never deployed** → `supabase.functions.invoke` 404'd, error swallowed (best-effort), so invitees got no email. Deployed it (`supabase functions deploy send-group-invite`; Docker-less API upload; RESEND_API_KEY already set, sends from `invites@sidebar.watch` like the others). `[[reference_db_backup]]` covers the no-Docker dump path.
> - **Beta gate rotated** (new password via `UPDATE beta_config SET hash = extensions.crypt('…', extensions.gen_salt('bf'))`) + **forced re-auth** by bumping the localStorage key `ns_beta_access → ns_beta_access_2` (`BetaGate.tsx`).
> - **Invite flow:** full-page **invite arrival** screen (reuses `PublicDashboardPage` in a new `invite` mode — wants-first headings + "Want to watch with them?/JOIN IN" footer, no back arrow; watching pills show real progress via the public-pool data, no migration); pending-invitee cluster avatar = yellow fill / **white** letter (email's first char); **"no" on the join prompt now DECLINES** (deletes the invite → leaves both rails); red invited-cluster avatar = **cream** letter + 2-line hover tooltip "You've been invited by @X…"; confirmation modal is just **"Invites sent!"** (link/copy dropped); CP5a dev copy removed.
> - **Dashboard / room polish:** empty shelf labels hidden; cluster avatars 40px with 4px gaps; **TVMaze add-show RESTORED** in dashboard search (catalog-only search was a regression — `tvmazeSearch`+`createShow` re-wired, no Supabase egress); map green dot now **own-column only** (was a bug flooding every cell); map dots clip behind the sticky header (z-index); ticket flash fires after the scroll lands; currently-watching tooltip gained a watch-gap line; chat "You're connected with:" label blue (names stay green). Show-layout rule (1–2 centered, 3+ 3-col) applied to **public pool** + **invite arrival** too; public pool heading "@user's watch pool:" + colon-less shelves.
> - **Dynamic logo** (`SidebarLogo.tsx`): blocks now **opaque** (no mix-blend, §16 simple-overlaps); the sky cell shows the OPPOSITE of its background via an explicit **`bg` prop** ("green"|"sky") — NOT the body class (the show-room body palette is sky but its header is green, which mis-flipped it). Green home → sky cell; group-context dashboard (`bg="sky"`) → green cell; show room (default) → sky cell.
> - **Compose modal restyle** (`v2/ComposeForm.tsx` + ShowRoomPage `composeCloseX`): **destination chooser + explainers REMOVED** — destination is fixed by the tab (friend → friend room / "share entry"; private → "save privately"); title + "capture your thoughts on:" blue; 2px blue text-field outline; body 3 lines taller; placeholders yellow+opaque (needed `!important` to beat theme.ts `textarea::placeholder{color:#aaa!important}`); filled text midnightblue; "not now" + × red (Lucide X 2px); "want a prompt?" Body font universally (shared `.prompt-btn` class + inline); discard modal cream/no-outline, Header 2 + Body + midnightblue. **Note:** chooser removal is global — only affects the orphaned dormant `/compose` route (slated for deletion).
> - **OPEN:** the cutover's "delete dormant legacy code once proven" is still pending tester sign-off; `backup-precutover.sql` kept as the restore point.

> **2026-06-24 — RESTRUCTURE CUTOVER COMPLETE. The new world is now THE site.** The full open spec shipped + cut over in one session (TSP demo + CP5b + CP7 + CP8). The old `/journal` / `/profile` / public-profile / legacy-room surfaces are **retired** (desktop redirects into the new world; code kept dormant, not deleted). Mobile (`/m`) untouched. **Migrations applied to prod (in order):** `20260623_restructure_cp5b_invite_inviter_shows.sql`, `20260623_restructure_cp8a_migration_dormant.sql`, then the one-time destructive `20260624_restructure_cp8b_cutover.sql`. `send-digests` edge fn **redeployed** (links now point at `/show-room`). A full pre-cutover dump (`backup-precutover.sql`, ~1.4 MB, kept off-repo) was taken first. **Further tester verification happening 2026-06-25+.** Notes refreshed: `[[project_restructure_spec]]`, `[[project_tsp_onboarding_demo]]`, `[[project_digest_deeplink_cutover]]`.
> - **CP5b — account creation from a group invite link.** A logged-out invitee opening `/group-invite/:token` now gets through the beta wall (that one route is exempt), sees a rich welcome (inviter's shows + email pre-filled/locked), creates an account, and lands on `/dashboard` where the guided tour auto-fires — finally fulfilling the TSP demo's "right after signup" placement. Migration adds the inviter's shows + invitee email to the token-scoped lookup. New world signup routing forks ONLY invite signups (`sessionStorage ns_post_signup_dest`); general signup path was untouched at the time, then universalized by CP7. Touched: `BetaGate.tsx`, `AuthModal.tsx` (`initialEmail`/`lockEmail`/`onSuccess`), `GroupInviteAcceptPage.tsx`, `db.ts`, `App.tsx`.
> - **CP7 — navigation cutover (desktop).** Every desktop sign-in + the `/` bounce now go to `/dashboard`. Legacy routes redirect (desktop only; mobile falls through to `/m` untouched): `/journal`→`/dashboard`, `/profile`→`/dashboard`, `/u/:username`(+aggregate)→`/pool/:username`, `/room/:id` & `/v2/room/:id`→`/show-room/:id` (preserving `?entry=`), `/invite/:token`→`/dashboard`. Old components kept dormant (reversible; delete-later when Alborz is "absolutely sure"). **Digest deep-links:** `ShowRoomPage` now reads `?entry=` and auto-expands the post; `send-digests` repointed to `/show-room`. Touched: `App.tsx`, `ShowRoomPage.tsx`, `supabase/functions/send-digests/index.ts`. Guard via `onMobile = innerWidth < 768`.
> - **CP8a — BTU migration-dormancy flag (additive).** `people_group_members.migration_dormant` + `clear_migration_dormant_for_show(show)` RPC. Dashboard hides a dormant group from the rail and un-hides it on the show's re-add (`fetchPeopleGroupsForUser` filters it tolerantly; `addShow`/`restoreShow` call the clear + reload rail). Touched: `db.ts`, `DashboardPage.tsx`.
> - **CP8b — the one-time destructive cutover (`20260624_restructure_cp8b_cutover.sql`, APPLIED).** Single transaction, hard Step-0 assertions (aborts on BTU/Paradise/seed mismatch). Wiped every account except the 5 BTU users (`Jill`, `libdenk`, `matthewgarcia80`, `christinelovesshows`, `albokay`) + the parked TSP seed; reshaped BTU's Paradise into a removed-from-pool "Beyond the Underdome" people-group (the existing friend room re-parented as its 1 show room, all writing/map/ratings intact); BTU set out-of-pool + dormant + onboarding reset (so they re-add Paradise and get the tour); dropped the on-signup `on_new_profile_provision_sidebar` TSP trigger (function + seed PARKED). **Verified end state: 8 profiles (5 BTU + 3 fictional seed authors — seed are profile-only, so 5 logins), Beyond the Underdome with 1 room.** **Gotchas hit + fixed live (the file is the corrected version):** deletes must be FK-safe — the only RESTRICT blockers are `likes→replies/threads`, `replies→threads`, `{threads,replies,progress,likes}→profiles`; a kept reply/like CANNOT outlive a doomed parent thread (compute doomed thread+reply sets once, delete likes→replies→threads→progress, then profiles cascades the rest, then `auth.users`). Re-run is a safe no-op (Step 0 fails on the now-parented room). **Backup approach:** no Supabase auto-backups (free-tier-ish; DB only 0.03 GB), so a manual `pg_dump` via `brew install libpq` (`/opt/homebrew/opt/libpq/bin/pg_dump`, Session-pooler conn string) — the CLI's `db dump` needs Docker (absent), the direct `pg_dump` does not.
>
> **OPEN / next:** tester verification of the live cutover (2026-06-25+); delete the dormant legacy components once proven; `backup-precutover.sql` kept as the restore point meanwhile. Optional follow-ups still standing: `[[project_focus_refresh_followup]]`, `[[project_realtime_room_map_followup]]`, `[[project_departed_writer_shelf_followup]]`, `[[project_error_tracking_followup]]`, `[[project_pre_beta_checklist]]`.

> **2026-06-23 (follow-up 3) — Restructure odds-and-ends batch (4).** Small polish, all on the restructure surfaces (live site untouched). No migrations. Touched: `ShowRoomPage.tsx`, `DashboardPage.tsx`. **Kicks off the open spec** (finalize TSP demo + CP5b/CP7/CP8 still ahead — `[[project_tsp_onboarding_demo]]`, `[[project_restructure_spec]]`).
> - **Show-room order dropdown → single "episode order"** (`ShowRoomPage.tsx`): the Sort group lost "oldest first"; the lone option "episode order" = newest→oldest (the existing `sort:desc`). Per-member filters unchanged (already force desc).
> - **"just log my progress" → "just confirm my progress"** (`DashboardPage.tsx`): all three modal buttons (solo / watchq / personal-watching) + the matching code comment.
> - **Group-room solo heading is progress-aware** (`DashboardPage.tsx`): the `clicked.mode === "solo"` pop-up reads **"Have you started watching?"** when your current progress is S0E0 (the "haven't started yet" shelf), else "Have you watched more?" (brings the group room in line with the personal dashboard, which already did this).
> - **Live chat dot while viewing a group, panel CLOSED** (`DashboardPage.tsx`): new effect keyed `[activeGroupId, chatGroupId, selfUserId]` — a filtered per-group `group_messages` INSERT subscription (same auth handling as the open-chat socket: `realtime.setAuth` for member-gated RLS) that, on another member's message, **bumps that group's `chatActivity.latestMessageAt` locally → flips the blue dot — NO refetch, nothing polls** (egress = one small INSERT push for the single open group). Skipped while the panel is open for that group (`chatGroupId === activeGroupId`) — the open-chat effect already streams + marks seen; tears down on leaving the group / opening chat. Channel `group-chat-dot-<gid>`.

> **2026-06-23 (follow-up 2) — TSP onboarding demo (now LIVE on /dashboard) + auth tab-focus guard.** Built from `~/Downloads/sidebar_tsp_onboarding_demo_spec.md` (`[[project_tsp_onboarding_demo]]`). **Alborz is continuing this spec in a fresh session.**
>
> **TSP onboarding demo** — a once-only guided tour of a sample show room, **the restructure world's post-signup onboarding**. Fully **in-memory**: no DB content, no `provision_sidebar_protocol`, nothing written except the once-only flag. Reuses the REAL room components in a new **demo/controlled mode** (drift-resistant). **ENABLED:** auto-shows once on a user's first arrival at the **base `/dashboard`** (skips `?g=…` group contexts), gated by the durable `profiles.tsp_demo_seen_at` flag — migration **`20260623_profiles_tsp_demo_seen.sql` APPLIED to prod by Alborz this session.** **Does NOT change live signup routing** — general new sign-ups still get the old onboarding → `/journal` (live `/journal` world untouched); only people who reach `/dashboard` (e.g. accepting a group invite — `GroupInviteAcceptPage` routes to `/dashboard`) see it. `/dashboard?tspdemo=1` force-shows for testing WITHOUT marking seen (reset your own real flag with `UPDATE profiles SET tsp_demo_seen_at = NULL WHERE id = '<id>'`).
> - **New files:** `src/lib/tspDemoFixture.ts` (baked-in fixture: verbatim Maya/James/Sarah `tsp-seed` entries + replies with episode tags, 7 instructional "Alborz" entries `isInstructional`, 3 fixed map columns Maya E2 / James E4 / Sarah E6 + ratings); `src/components/TSPDemoModal.tsx` (orchestrator: compose-modal cream frame [85vw×90vh, 64px even borders], centered eyebrow/Lora title, progress dropdown atop a **sky room panel**, reused feed+map, fixed header/footer with **sticky map**, **scroll-to-top on progress change**, single two-state exit control "skip the tour" → "start finding shows →" latched once E6 reached; first guide entry expanded by default).
> - **Controlled mode on reused components:** `V2RoomFeed` gained `demoMode` + `demoReplies` + `isInstructional` (on `V2RoomFeedEntry`): read-only expanded view = full body + per-reply-gated replies inline (ahead-of-progress ones render the live `card.redacted` "<author> responded from episode S# E#." stub), **no V2InlineThread → no DB fetch / composer / likes**; guide entries get a cream Flag leading the title, cream Sparkles REPLACING the avatar by the byline, blue title text, a blue `.tsp-guide` outline (CSS in `theme.ts`, since group-context forces the ticket border `!important`), a live-style bottom-only collapse button, a trailing "…" on collapsed cards, and `keepTailTogether` title-wrap (last 3 words nbsp-joined). `OneSelectProgress` gained `forceZeroOption` (keeps "haven't started" pickable so the demo moves freely up/down — off for every live caller).
> - **Mechanics:** gate = `entry.episode <= selectedEpisode` on LOCAL state (applies to replies too) — never the live spoiler engine; demo dropdown is **non-monotonic** (lowering re-hides + shrinks your own column); **no rating-capture confirm modal** (uses the selection path, not forward-pick; map self-edit → local ratings). `db.ts`: `fetchTspDemoSeen`/`markTspDemoSeen` (tolerant). `DashboardPage` mounts it behind `TSP_DEMO_ENABLED` (now `true`). **Live rooms + live site untouched** — every demo path gated behind `demoMode`/`isInstructional`, off everywhere else.
> - **OPEN (next session):** account-creation-directly-from-an-invite-link (spec CP5b) isn't built — today a brand-new invitee is told to sign in first (`GroupInviteAcceptPage` header note); when built, route the new account to `/dashboard` and the demo becomes their first experience automatically (no demo work needed). Couldn't self-verify visuals (preview skipped for Sidebar) — confirm the sticky map, reply stubs, and icon contrast on the deploy.
>
> **Auth tab-focus guard** (`src/lib/auth.tsx`): `onAuthStateChange` swapped a fresh `user` object + reloaded the profile on EVERY auth event — incl. the TOKEN_REFRESHED/redundant-SIGNED_IN Supabase fires when a tab regains focus — cascading into every screen re-running its full bootstrap ("click back into the tab → page flashes green → group room re-renders", plus a per-refocus re-fetch egress burst). Now it reacts only when the signed-in identity actually changes: same-user events keep the exact same `user` reference + skip the profile reload (also dedupes the initial getSession + INITIAL_SESSION double profile-load). **Tradeoff:** switching back to a tab no longer auto-pulls fresh data (seen on next nav; chat stays live via its own realtime) — optional future quiet on-focus refresh noted (`[[project_focus_refresh_followup]]`). Touches the SHARED auth layer (live site + new world).

> **2026-06-23 — Vote-opt-in fix, dashboard polish batch (8), egress cache, group-pill ▼.** All on the restructure surfaces (live site untouched). **Migrations (apply to prod):** `20260622_people_group_seq.sql` — adds `people_groups.seq` (stable per-creator group number), backfills, stamps it in `create_people_group`; powers the "Group N" naming. (`20260622_group_dashboard_respect_in_pool.sql` from the prior entry already applied.) Touched: `DashboardPage.tsx`, `v2/ComposeForm.tsx`, `ShowRoomPage.tsx`, `lib/db.ts`, `lib/groupPills.ts`, `types.ts`; new migration; memory notes; HANDOFF.
> - **Voting "yes" opts you into your pool (BUG FIX):** writing in a show room needs the show in your personal pool (the composer requires a `progress` row, else the legacy "no journal tab for this show" message). Adding a show to want-to-watch created that row; voting "yes" did not — so opted-in-by-vote members couldn't write while the adder could. `doVote` now, on a "yes" vote, creates a not-started (S0E0) want-to-watch `progress` entry when you have none (same row the adder gets), or restores the show to your pool if you'd previously removed it (otherwise voting back in left you opted-out everywhere, given the in_pool fix). Real progress is never reset. Frontend-only.
> - **Dashboard polish batch (8 items):** (1+2) **group clusters** show every OTHER person's icon (accepted cream / not-yet-accepted invitees yellow), **never your own**, stacked in a pyramid (1, 2 side-by-side, 3 triangle, growing to the 8-cap via `pyramidRows`/`AvatarPile`). (6) **unnamed groups → "Group N"** (stable per-creator `seq`) in the cluster + group header; member handles stay in the "with" line; pre-migration falls back to handle-names. (4) **vote toggle** restyled: colored sliding dot (yellow no / green yes), only the active label shown in cream. (5) **"not yet"** cream-outline button beside "Yes" in the start-a-room reveal. (3) **"haven't started yet" buttons** tooltip which friends are also interested (italic show name; the cursor-tooltip `tip.text`/`sub` became `ReactNode`; "also" only when YOU'VE opted in too, keyed off `selfOpted`; 1 / "X and Y" / "N friends"). (8) **lone-writer icon** tooltip gains "They have started writing in here." (7) **compose default destination** matches the tab you wrote from: friend tab → this room, private tab → private (new `defaultDestination` prop on `ComposeForm`). **Chat tab intentionally left untipped.**
> - **Egress — shared shows-catalog cache:** `fetchShows()` was re-downloading the entire catalog (every show's full season data) on nearly every screen load AND every dice/progress action in a show room — a major egress driver as restructure testing ramped (the user was over the 5GB free tier again, bursty heavy days). Now the first `fetchShows()` caches the catalog in memory + keeps it fresh via a realtime subscription (`shows-catalog-cache`); every later call reads the shared copy with NO network. `createShow`/`refreshShowIfStale`/`adminDeleteShow`/`adminToggleHidden` patch the cache in place (+ `invalidateShowsCache()` export). Page reload re-fetches. **`App.tsx`'s own shows copy + realtime left untouched** (live site safe; it shares the first download via the cache). Net: ~one catalog download per page load instead of per screen + per interaction. Folds in the "fetch one show" / "don't refetch in-room" goals (reading the shared copy beats a per-show network fetch). **Verify by watching the egress-per-day chart drop over the next days.**
> - **Group pill ▼ for not-started viewers (`groupPills.ts`):** a show button where YOU hadn't started but a friend had was parroting the friend's raw s/e (e.g. "s2 e8"), which read like your own progress. Now a not-started viewer gets a red **▼** for how far behind the **furthest-along** watcher they are (your progress = effectively S0E0). Watching cases (▲ ahead, matched-progress s/e) and the written-but-unwatched "s0 e0" are unchanged.
> - **New memory notes this arc:** `[[project_departed_writer_shelf_followup]]` (remove-from-pool fully opts you out incl. the "who's writing" indicator — revisit after testers), `[[project_digest_deeplink_cutover]]` (CP7: repoint digest links to `/show-room` + teach `ShowRoomPage` `?entry=` auto-expand).
> - **OPEN / TODO:** apply `20260622_people_group_seq.sql` (Group N inert until then); eyeball the cluster icon-pyramid spacing on the live deploy (gap/overlap are best-guess pixel values, tunable in `AvatarPile`); confirm the egress drop on the chart.

> **2026-06-22 (follow-up) — Dashboard/group tooltips, chat autoscroll, map-cell opacity, remove-from-pool opt-out fix.** Small polish + one bug fix, all on the restructure surfaces (live site untouched). **Migration applied to prod by Alborz this session:** `20260622_group_dashboard_respect_in_pool.sql` (`get_group_dashboard` now honors `progress.in_pool`). Touched: `DashboardPage.tsx`, `v2/V2RoomMap.tsx`; new migration; HANDOFF.
> - **New-activity dot tooltips (DashboardPage):** the cursor-following watch-progress tooltip on group show buttons now hangs the new-activity notification line beneath a divider — blue/visible = "There is new writing in here for you.", red/invisible = "…for when you catch up." (notification-only when no progress is logged). Group clusters get the same line on hover (blue = any new visible activity incl. chat; red = invisible). **Chat tab intentionally left untipped** (Alborz's call). Show-button dot made `pointer-events:none` so the tooltip fires through it; cluster native `title` suppressed while the tooltip is up. Strings reuse the live room/feed copy. New module consts `NOTIF_VISIBLE`/`NOTIF_INVISIBLE`; `tipProps(progress, notif)` + `tip.sub`/`tip.wrap` + `tipDivider`; `roomNotif(roomId)`; `GroupClusters` gained an `onTip` prop.
> - **Group chat autoscroll:** the chat body stays pinned to the newest message (new `chatBodyRef` + effect on `chatMessages`) — on open, on send, and on an incoming realtime message while the panel is open.
> - **Copy:** solo/other pill modals now read "Open show room?" (added "?"); the remove-from-pool modal reworded to "This will opt you out of the show across all your groups and you will leave any friend rooms for the show." (rest of that modal's copy unchanged).
> - **Map hidden-cell opacity (`V2RoomMap`, shared → BOTH the live friend room and the new show room):** the hidden/ahead-of-progress cell fill was translucent `var(--dos-border)` (= `rgba(26,58,74,0.3)` in group-context), so a multi-entry stack's offset receding back layers bled through the top cell and read as transparency. Swapped to the **opaque** equivalent `#819dad` (that same greyblue composited over the friend-room bg `#adc8d7`) — a lone hidden cell and the receding back layers (still `opacity:0.3`) look identical; only the top of a stack is now fully solid.
> - **Remove-from-pool opt-out (BUG FIX):** removing a show from your dashboard pool kept showing you as opted-in inside group rooms. Root cause: `get_group_dashboard` derived the group pool + member opt-in from votes OR ANY leftover progress, **ignoring `in_pool`** — and since remove-from-pool keeps progress (so it can be restored), your progress row re-surfaced you. The server cascade (`remove_show_from_pool`: delete votes, leave the show's rooms, insert `friend_group_departed_members` termination dot) was working; only the dashboard read re-added you. Fix (migration above, read-only `CREATE OR REPLACE`): out-of-pool progress no longer counts toward the group pool, and a member who has removed a show is excluded from that show's member list entirely (even if they wrote in the room — remove-from-pool is the definitive opt-out). Written content is untouched; the map termination dot is unaffected. **Nuance flagged:** the shelf's "who's writing" indicator no longer counts a departed member — possibly revisit after tester feedback (`[[project_departed_writer_shelf_followup]]`).
> - **Digest deep-links folded into CP7 (gated):** at cutover the digest email's `/room/<id>?entry=<threadId>` links must be repointed to `/show-room` AND `ShowRoomPage` taught to auto-expand from `?entry=` (it has `feedRef.expandEntry`/`scrollToEntry` but doesn't read the param) so emailed entry links still open the specific thread — see the CP7 line in the entry below (`[[project_digest_deeplink_cutover]]`).
>
> **2026-06-22 — Restructure polish arc: dashboard/group-room chrome, opt-in avatars, activity ordering, progress dropdowns, realtime chat, new-activity dots, in-room map notifications, full show-room parity, public dashboard.** Long multi-push session, all additive on the new restructure surfaces (live site still untouched). **Migrations applied to prod by Alborz this session (in order):** `20260620_restructure_group_dashboard_activity.sql` (get_group_dashboard returns per-show `last_activity_at` = newest room writing + member progress update), `20260621_progress_updated_at.sql` (**adds `progress.updated_at` column + BEFORE-UPDATE trigger** — the table had NO timestamp; this also un-broke a regression, see below), `20260621_group_chat_realtime.sql` (adds `group_messages` to the `supabase_realtime` publication), `20260621_room_activity_exclude_own.sql` (`get_room_activity_visibility` gains `p_exclude_own BOOLEAN DEFAULT false` + returns `parent_group_id`; DROP+CREATE), `20260621_group_chat_last_seen.sql` (`people_group_members.chat_last_seen_at` + `mark_group_chat_seen` + `get_group_chat_activity`), `20260622_room_activity_invisible.sql` (`get_room_activity_visibility` also returns `latest_invisible_activity_at` = others' ahead-of-progress writing; DROP+CREATE). **New file:** `src/components/PublicDashboardPage.tsx` at route `/pool/:username` (added in App.tsx top-level short-circuit). Touched: `DashboardPage.tsx`, `ShowRoomPage.tsx`, `v2/V2RoomFeed.tsx`, `v2/ComposeForm.tsx`, `styles/theme.ts`, `lib/db.ts`, `types.ts`, `NudgePopover.tsx`, `App.tsx`.
>
> **Chrome / layout (DashboardPage + ShowRoomPage):** Group-context back-arrow became a cream left-edge tab mirroring the chat tab (chat moved lower); group header got a greyblue "with" before member names. Show room: `×` replaced by a left-edge back-arrow tab → group; tab labels are **Inter 17/600 ("Header 2"), cream in both states**; header+body colors **swap by tab** (friend = green header/sky body, private = sky header/green body); panel/selected-tab cream outlines dropped (deselected keeps one); body now mirrors the live room's centered **672 feed column + sticky map within max-1400** (write+progress live atop the column); the **private-writing tab renders through the same `V2RoomFeed`** as the friend feed (groupId-less = public mode) so private posts expand identically. Entry tickets are **2px cream**, star buttons cream (group-context CSS, both rooms).
>
> **Opt-in avatars (group show buttons):** members who opted into each show render as **avatars overlapping the button's bottom edge, right-anchored** (cream ring, blue initial), excluding you; the green count badge + in-button writer-pencil were **dropped** (2+ writers people-icon kept). A **single writer** is shown as **green fill + pencil badge on that writer's avatar** (nothing if the lone writer is you). Cursor-following **green tooltip** (`@user has watched: Sx Ex`) on avatars; show button shows `You've watched: Sx Ex` when you have progress. Search now lists already-in-pool shows as a muted "You've already added <i>X</i> to your watch pool."
>
> **Activity ordering:** Group "currently watching" shelf is bucketed (2+ writing → 1 writing → 2+ watching → 1 watching) then by most-recent activity (`last_activity_at`). Personal dashboard orders show buttons by latest progress update (`progress.updatedAt`, exposed via `fetchProgress` selecting `updated_at`). **REGRESSION FIXED:** an earlier commit referenced `progress.updated_at` (in `fetchProgress` AND the group RPC) before the column existed → `fetchProgress` threw (it was added to BOTH the primary and legacy fallback selects, killing the safety net) → the whole dashboard `Promise.all` rejected → all shows blanked. Fix: `progress.updated_at` migration + `fetchProgress` legacy fallback no longer requests it (and only passes `p_exclude_own` to the room RPC when true, so pre-migration callers keep the old signature). **Lesson reinforced: verify a DB column exists before referencing it.**
>
> **Progress dropdowns on pill click (update progress WITHOUT entering a room):** Personal dashboard — currently-watching pill → "Have you watched more?" picker + "Do you want to write by yourself? [Yes → private standalone] [just log my progress]"; haven't-started pill → "Have you started watching?" self-committing picker. Group room — your-own-show ("solo") modal gained the same "Have you watched more?" picker + log option; the also-watching modal already had a picker (+ "just log my progress"). All pickers default to current progress so a button press can't reset it; `logProgressPersonal` preserves highest-watched.
>
> **Real-time group chat:** filtered Supabase `postgres_changes` subscription on `group_messages` scoped to the open `group_id` (append-only, dedupe by id), mounted while the chat panel is open; keeps fetch-on-open/refetch-on-send as a pre-migration fallback. **Gotcha (don't re-discover):** `group_messages` is member-gated RLS, so the realtime socket must carry the user's token — supabase-js auto-wiring missed restored sessions, so the effect calls `supabase.realtime.setAuth(session.access_token)` before subscribing. The app's other realtime (ShowSection replies) is on anon-readable rows, which is why authed realtime had never been exercised. Room-feed + map realtime is the noted next candidate (`[[project_realtime_room_map_followup]]`).
>
> **New-activity dots (blue + red):** reuses the shared `get_room_activity_visibility` engine (own-excluded variant). **Blue** = new VISIBLE writing; **red** = new INVISIBLE (ahead-of-progress) writing; **visible takes priority** (per-room + per-cluster). On group show buttons (overlapping top-left), the chat button (blue, new messages), and dashboard group clusters (left of the name; rolls up room writing OR chat). Cleared on **room entry** (`markRoomSeen` in ShowRoomPage `load`) / **chat open** (`markGroupChatSeen` + optimistic), cascading to the cluster (entering clears both blue+red since they share `last_seen`). Chat last-seen is server-side (`people_group_members.chat_last_seen_at`). Dot data loads tolerantly (no dots if a migration's missing; never blanks the page).
>
> **In-room map notifications ported into ShowRoomPage** (were entirely missing — ShowRoomPage reused `V2RoomMap`/`V2RoomFeed` but none of the signal orchestration). Brought over green/yellow/red `cellSignals` + white-"new" `isNewMap` + their state (`lastOpenedAt`/`ns_highlight_seen`/`ns_tdot_dismiss` hydration, hidden-reply data from the existing `fetchGroupThreads`, the highlight-signal fetch for yellow) + expand/collapse/dismiss/cell-click handlers, wired to feed + map.
>
> **Full show-room parity with the live friend room (3 passes), EXCEPT two intentional differences:** member-name clicks go to the new **public dashboard** (`/pool/:username`, read-only show pool + sign-in CTA for logged-out + back tab), not the old profile; and the redesigned header chrome is kept (not reverted). Parity fixes: **map-cell click now scrolls + flashes** — `V2RoomFeed.scrollToEntry` gained an optional `scrollContainerRef` because ShowRoomPage scrolls inside a `position:fixed` wrapper (window-scroll was a no-op there); the multi-post cell hierarchy already lived in `V2RoomMap`. Feed defaults to **newest-episode-first** + a **sort / member-filter dropdown**; **map is sticky**; the **Poll/SIKW/IncomingPing stickies** now render; edits/deletes reflect in the feed; publishing auto-expands the new entry; poll-open refresh wired. The **map-click highlight is now a midnight-blue flashing dropshadow** (`flash-glow` keyframe), not a border color flash — the (cream) outline never changes color (the live `flash-border-blue` path is retired in V2RoomFeed; live site is dormant so not scoped). `NudgePopover`'s "view profile" link also repointed to `/pool/:username`.
>
> **REMAINING (unchanged from the restructure plan — both still GATED):** (a) ongoing aesthetic adjustments; (b) **CP7 — GATED:** collapse the V1/V2/V3 generational layering + redirect the legacy live routes (`/journal`, `/profile`, public profiles, `/v2/room/:id`, `/invite/:token`, and now `/u/:username` → `/pool/:username`) into the new world (§15.6) — deletes/redirects live surfaces, confirm per §0 gate 1; **also at CP7 — digest-email deep-links:** the daily friend-room digest (`send-digests` edge fn) links each entry as `/room/<groupId>?entry=<threadId>`, which the LEGACY room (`V2FriendRoomPage`) auto-expands via its `initialExpandThreadId`/`?entry=` reader. On cutover these emailed links must still open the specific thread, so (1) repoint `send-digests`' URL to the new room route (`/show-room/<roomId>?entry=<threadId>` — the ids line up, a parented `friend_groups` id IS the show-room roomId) OR add a `/room/:id`→`/show-room/:id` redirect that preserves `?entry=`, AND (2) bring the `?entry=` (+ router-state) auto-expand pattern over to `ShowRoomPage`, which already has `feedRef.expandEntry`/`scrollToEntry` handles but does NOT yet read the param on mount. Redeploy `send-digests` after (edge fns don't deploy on git push). Confirmed needed 2026-06-22 (`[[project_digest_deeplink_cutover]]`). (c) **CP8 — GATED, DESTRUCTIVE:** the one-time cutover migration + BTU island (§15) — wipe all non-BTU/non-seed accounts, reshape the 5 BTU users' Paradise into "removed-from-pool" ("Beyond the Underdome"), `migration_dormant` flag, TSP teardown. Needs a **full DB backup + explicit go/no-go from Alborz** immediately before running. See `[[project_restructure_spec]]` + `~/Downloads/sidebar_restructure_spec.md` §15.

> **2026-06-20 — IN PROGRESS: the big restructure (groups + show rooms).** Ground-up pivot from `~/Downloads/sidebar_restructure_spec.md` (+ visual deck `sidebar RESTRUCTURE 0619 03.pdf`, appearance-only). The product reorients around **people-groups** (a group is a persistent set of PEOPLE spanning shows) and **show rooms = (group × show)** (exactly one per pair, fully independent across groups). Everything below is **additive and behind NEW routes** — the live site (`/journal`, `/show/:id`, `/profile`, legacy `/room/:id`, etc.) is **completely untouched**; the restructure has **NOT been cut over** yet. Pushed to `main` (auto-deploys), so it's testable on prod at the new routes.
>
> **New routes (all top-level short-circuits in `App.tsx`, outside AppShell):**
> - `/dashboard` → `src/components/DashboardPage.tsx` — the new green home (search-first pool + two shelves) AND, when `?g=<peopleGroupId>` is set, the sky **group context** (pooled shelves + §7 pill system + §9 click model). Groups render as avatar **clusters at the top of the body** (the old right rail was removed). Chat opens via a partial pill at the right edge in group context.
> - `/group-invite/:token` → `src/components/GroupInviteAcceptPage.tsx` — people-group invite accept (separate from legacy `/invite/:token`).
> - `/show-room/:roomId` → `src/components/ShowRoomPage.tsx` — the two-tab room (friend room / private writing). Reuses `V2RoomFeed` + `V2RoomMap` (feed/map/dice/nudges) re-pointed to the room; in-room progress picker → rating capture; compose reuses the real `ComposeForm` constrained via a new `restrictGroupId` prop (destinations limited to THIS room + private — no public/other groups).
>
> **New files:** `DashboardPage.tsx`, `GroupInviteAcceptPage.tsx`, `ShowRoomPage.tsx`, `src/lib/groupPills.ts` (pure §7 pill-state logic — counts/fill/pencil/people-icon/▲▼/shelf), `supabase/functions/send-group-invite/` (Resend email for group invites). Reused/extended: `ComposeForm.tsx` (+`restrictGroupId`), `db.ts` (many new accessors), `types.ts` (PeopleGroup/PeopleGroupMember/GroupShowVote).
>
> **Data model (all NEW tables, additive):** `people_groups`, `people_group_members` (user_id → **auth.users**, NOT profiles — resolve usernames via a 2nd query, the PostgREST `profiles(username)` embed has no FK to follow), `group_show_votes` (per group×user×show opt-in), `people_group_invitations`, `group_messages` (chat), `group_show_votes`; plus columns `friend_groups.parent_group_id` (a parented friend_groups row IS a (group×show) room — reuses all existing room machinery) and `progress.in_pool` (remove-from-pool keeps progress but hides it; the live `fetchProgress` does NOT select it, read separately/tolerantly).
>
> **Migrations — applied to prod by Alborz, in this order** (all `supabase/migrations/2026061*_restructure_*` + `20260620_restructure_phase9_*`): phase1 people_groups → **phase1b** (fixes self-referential RLS recursion on people_group_members via the `is_member_of_people_group` SECURITY DEFINER helper — RLS policies on that table MUST use the helper, never self-query) → phase2 group_dashboard_rpc (`get_group_dashboard`) → phase3 group_invitations → phase4 group_lifecycle (leave/rename/pending) → phase5 group_chat → phase6 remove_from_pool → phase7 start_room_rejoin (re-join + clear dot when opening an existing room) → phase8 member_cap (8 max) → phase9 group_avatar_states (pending-invitees + group_name on my-invites). **Edge function `send-group-invite` still needs `supabase functions deploy send-group-invite`** (reuses RESEND_API_KEY; until then the in-app invite **link** is the fallback). `config.toml` has its `verify_jwt = false` entry.
>
> **Checkpoints done:** dashboard, group context (pills/voting/click-model), invites (link + in-app "*you're invited" + real email), two-tab room (write/feed/map/dice/nudges/progress/ratings), group leave/rename, chat, remove-from-pool, 8-cap, connect-more-friends. Plus review-driven UI passes (clusters-to-top, opted-in tooltip, button-outline rule, group heading redesign, no -2 letter-spacing globally).
>
> **REMAINING (next session):** (a) further aesthetic adjustments; (b) **CP7 — GATED:** collapse the V1/V2/V3 generational layering + redirect legacy routes (`/profile`, public profiles, `/v2/room/:id`, `/invite/:token`) into the new world (§15.6) — deletes/redirects live surfaces, confirm per §0 gate 1; (c) **CP8 — GATED, DESTRUCTIVE:** the one-time cutover migration + BTU island (§15) — wipe all non-BTU/non-seed accounts, reshape the 5 BTU users' Paradise into "removed-from-pool", `migration_dormant` flag, TSP teardown. Needs a **full DB backup + explicit go/no-go from Alborz** immediately before running. See `[[project_restructure_spec]]` memory + `~/Downloads/sidebar_restructure_spec.md` §15.
>
> **Gotchas hit (don't re-discover):** RLS self-recursion (phase1b); the profiles-embed FK gap; `OneSelectProgress.onConfirm` no-ops when `requireConfirm={false}` (use `onChangeSelected` for tracking, `onForwardPick` for the rating-capture handoff); `start_show_room` must re-enroll on existing-room open; the new dashboard must read `in_pool`/people-group data **separately + tolerantly** so it never breaks `fetchProgress` for the live app.
...
> **2026-06-17 — LANDED: First-login onboarding (paged modal + self-assembling reveal + TSP surfacing + "no room yet" empty state).** From `sidebar_spec_onboarding_v03.md`. Two pushes: CP1/CP2/CP4/CP5 first, then **CP3 (the reveal)**. All five checkpoints shipped. **Durable gate (CP1):** new `profiles.onboarded_at timestamptz` — migration `20260608_profile_onboarded_at.sql` **MUST be applied in Supabase before/with deploy** or every user loops through onboarding (loadProfile reads it via a new column-tolerant cascade in `auth.tsx`; `markOnboarded` stamps it; `refreshProfile` added to `AuthProvider`/`useAuth`). Never-onboarded users route to `/profile` on login (post-login effect in `App.tsx` now waits for the profile row, then branches `onboarded_at == null ? /profile : /journal`; the `/`→ auth-gate branches the same way; **admins are exempt from the `/` bounce** but still see the modal if they open `/profile`). Existing testers are intentionally **NOT backfilled** (decision: everyone-not-yet-onboarded runs the flow). **Modal (CP2, `src/components/v2/OnboardingModal.tsx`):** portaled, cream 88vw×90vh (ComposeModal look). Forward-only pages **Canon→Thoughts→Watching-now**, bottom position-dots, **Confirm / "not now"** ("you can do this later." tooltip), locks on advance — no back/forward. **Canon** = 3 rows (show-search → blurb), heading `Shows that exemplify "good TV" for you.`; each picked show is `createShow` + progress set to the latest available episode (→ classifies *finished*) + **canon-pinned** (existing `canon_pin`) + optional `canon_take` blurb; Confirm enables at ≥1 row. **Thoughts** reuses `ProfileThoughtsCompose` inline (new backward-compatible `creamSurface` prop so the "post privately" button stays legible on cream) — **both private + public kept** (decision). **Watching-now** = stripped show-add (search + first/rewatch + progress), writes `shelf_override='watching'`, **no room creation**. TVMaze helpers + `EpisodeSelectInline` were **extracted from `SearchShows` into `src/lib/tvmaze.ts` + `src/components/EpisodeSelectInline.tsx`** (behavior-identical move; SearchShows now imports them — done to avoid risk-refactoring the sensitive SearchShows flow). On completion: `markOnboarded` → seed default TSP blurb (if unset) → `refreshProfile` → bump a `reloadKey` that re-runs `V2ProfileSelfPage`'s data bootstrap so the freshly-created shows/thought populate. **CP5 — TSP in Watching-now:** surfaces on the OWNER's profile once `onboarded_at` is set AND not stopped (stop-watching drops it off the profile entirely — never to the Stopped shelf; "graduates" past it); still hidden on `V2ProfileVisitorPage`; default editable blurb `"Want to see a room in action? …"` seeded at completion (only if unset, so a later clear sticks). Its card uses the generic watching-card path (name-link → journal tab; room CTA → TSP demo room via `fetchAllFriendGroupsWithActivity`). **CP4 — journal "no room yet" empty state:** new `EmptyProfileWelcome` `noRoomYet` variant (spec §7 copy) inserted in `V3JournalPage` **ahead of the legacy "haven't written for any friends yet" fallthrough**, fires when `tabGroups.length === 0` (not TSP, not invited, not self-created-room); CTA points at the existing "+ friends" toolbar button. **CP3 — self-assembling reveal (follow-up push):** after the modal's last advance it **fades out** (450ms; `OnboardingModal` gained a `closing` state + `onClosingStart` callback that flips the profile behind to "frame-only" before the fade ends → no flash). Orchestrated in `V2ProfileSelfPage` via a `revealStep` state (null = normal/all-visible; 0 = frame-only; beats 1=thoughts, 2=watching, 3=want, 4=finished, 5=top chrome). Shelf **headings stay visible (the frame)**; only each shelf's CONTENT is wrapped in a `revealStyle(beat)` opacity/translate fade. `startReveal()` is kicked from the data-bootstrap `.then` once the refreshed data lands (guarded by `pendingRevealRef`) and runs an **alternating action queue — fade · BEAT · scroll · settle · BEAT · fade · …** Each fade and each scroll is its own discrete action; the gap AFTER a scroll is `REVEAL_SCROLL_SETTLE_MS + REVEAL_BEAT_MS` (let the smooth-scroll fully stop, hold a beat, THEN the shelf fades) and the gap after a fade is `REVEAL_FADE_MS + REVEAL_BEAT_MS`. The **thoughts beat has no scroll** — it fades in with the page at the very top. Content fades use a **double-pulse keyframe** `reveal-pulse-in` (rise·dip·rise, settling visible — echoes the map-cell `flash-border-blue` highlight) rather than a plain opacity transition; a *cancel* still fades remaining content via a 500ms transition fallback. The pairedHeader fade (top chrome, last beat) uses a `pairedHeaderHidden` prop on `V2Layout`. **No pre-reveal flash:** the profile drops to frame-only (`revealStep = 0`) the instant the modal OPENS (not on close), and the hidden state is instant (no transition) — so existing content never shows through the translucent modal or fades OUT as it closes; the reveal only ever fades content IN. A separate `revealActive` flag (true only while the timed beats run) gates the interrupt listeners, so clicks/scrolls INSIDE the modal can't trip them. **No post-close pop-in / layout jump:** the modal's last advance awaits `onComplete` (stamp onboarded → seed TSP blurb → `refreshProfile` → refetch + apply profile data) **while the modal is still fully visible**, so the frame behind reaches final layout (stats line, zigzag, shelf heights) under the opaque modal BEFORE it fades; the modal then fades and fires `onRevealStart` (unmount + `startReveal`). The data fetch is factored into `fetchAllProfileData`/`applyProfileData`, shared by the mount effect and the completion path (no more `reloadKey`). Pacing/feel fully tunable via module constants at the top of `V2ProfileSelfPage`: `REVEAL_FADE_MS` (850) / `REVEAL_BEAT_MS` (100, pause after a fade before scrolling on) / `REVEAL_SCROLL_SETTLE_MS` (550) / `REVEAL_SCROLL_BEAT_MS` (300, pause after a scroll lands before its shelf fades) / `REVEAL_START_DELAY_MS` (400). **Interruptible — but deliberately desensitized** (an early build aborted on the faintest mouse jitter): cancel fires only on an intentional gesture — a `wheel` event whose `|deltaY| >= REVEAL_WHEEL_THRESHOLD` (28), a non-modifier `keydown`, or a `pointerdown` — AND only after a `REVEAL_GRACE_MS` (1000ms) grace window from the reveal start (touchstart dropped; site is desktop-only). We don't listen for `scroll`, so our own smooth-scrolls can't self-cancel; on cancel everything fades to visible. Runs for **EVERYONE incl. existing testers** (decision). **Also in CP3 — the "stay permanently" decision:** Watching-now + Finished shelves now **always render** (frame for the reveal) and show a **permanent "+ add" tile** when they have no real show — Watching keys off "no non-TSP show" (`add a show you're watching`), Finished off empty (`add a show to your list`); the single-open-tile state generalized from `addOpen` (bool) → `addOpenShelf` with a shared `renderAddTile(shelf,label)` helper. Reveal pacing + onboarding aesthetics flagged by Alborz for a later cleanup spec. **Minor open item:** the self-profile "N shows · M watching now" meta line still excludes TSP from its counts (cosmetic; a TSP-only user could read "0 watching now" with a TSP card visible) — left as-is pending a copy call. **2026-06-17 aesthetic pass:** modal content cluster vertically centered (margin-auto in a column-flex scroll body); bottom divider line removed; page-dots restyled to match the homepage "how it works" dots (10px circles, gap 8) but canon-yellow `#dea838` / faded `rgba(222,168,56,0.35)`, position-indicator only (not clickable); dynamic `SidebarLogo` added top-left INSIDE the modal, with the profile's chrome logo cross-fading IN as the modal fades OUT (new `chromeLogoHidden` prop on `V2Layout` + `onFadeStart` modal callback; `chromeLogoHidden` state in `V2ProfileSelfPage` set true on open, false on fade-start). Fields restyled to pill radius + no hard outline (`FIELD` / `SELECT_FIELD` + a shared `SelectedChip`; `EpisodeSelectInline` gained an optional `style` prop). Canon page: canon-yellow Lora welcome line ("Welcome to Sidebar. Let's set you up to put you in a TV mood."), heading → "Shows that exemplify the best TV has to offer.", subhead → "…pinned to your profile as your canon." Thoughts page in the modal **dropped the private/public buttons** → uses the standard Not-now/Confirm; **Confirm posts PUBLIC only** (private stays discoverable later on the profile) — `ProfileThoughtsCompose` gained `hideActions` + a `submitPublic()` imperative handle (now `forwardRef`). Watching-now: search field + episode dropdown pill-radii/no-outline, radio is a canon-yellow circle with a canon-**blue** `#355eb8` inner dot (the official canon blue; an earlier `#1a3a4a` read wrong), label → "What's the last episode you've watched?". **2026-06-17 follow-up fixes:** `REVEAL_BEAT_MS` → 300 (the after-fade pause before the next scroll, +50%); the modal's `SidebarLogo` wordmark is tinted canon-yellow via a new `wordmarkTint` prop (renders the wordmark PNG as a CSS alpha-mask over a `#dea838` box, width 165 = 52×3108/980 to keep proportions); both Canon-page header lines get `text-wrap: balance` (`.ob-balance`); welcome copy → "Welcome to Sidebar. Let's set you up to get you in a TV state of mind." **Bug fixed:** the Thoughts-page Confirm errored "add a body to your thought" even with text — `ProfileThoughtsCompose`'s `useImperativeHandle` had an empty deps array that froze the first-render `handleSubmit` closure (body=""); removed the deps array so `submitPublic` wraps the current-render closure. **2026-06-17 follow-up 2:** `SidebarLogo` gained an `arrangements?: Layout[]` prop (exported `Layout` type) overriding its random block-layout pool — the modal passes `MODAL_LOGO_ARRANGEMENTS` (2 layouts whose blocks all stay clear of the wordmark zone x:45–210/y:96+, so blocks never overlap the logotype in onboarding). Canon-page welcome split to two explicit lines ("Welcome to Sidebar." / "Let's set things up to put you in a TV state of mind."); heading → "Shows that represent the best TV has to offer." (the wordmark tint stays a CSS mask over `#dea838`; if the source PNG ever ghosts the "bar" glyphs it would read faded — swap in a flat canon-yellow logotype asset if so). **Chrome heading centered:** the "this is your public profile / go to your journal" pair (V2Layout pairedHeader) is now horizontally centered in the content column (`justifyContent:center` + `marginLeft:0` to drop the `.journalShift` +56px), matching the centered profile content; the journal's flipped pair ("this is your journal / go to your public profile" in `V3JournalPage`) got the same centered placement so the two surfaces' chrome sits in the same spot. (Legacy `ProfilePage` heading left as-is.)
>
