# Sidebar — Technical State (2026-06-28)

> **➡️ NEXT UP (review before building) — TRAILERS feature is PARKED with a finished, locked plan.** Alborz wants to re-review the plan with the next Code session BEFORE implementing. Everything needed is in memory `[[project_trailers_spec]]` + the spec `~/Downloads/sidebar_trailers_spec.md`. One-liner: a YouTube launch-trailer embed beneath the group opt-in modals, shown only for a not-started viewer; frontend-only EXCEPT it needs `VITE_TMDB_READ_TOKEN` in **Vercel** env. Live CORS/token check already passed (TMDB allows browser Bearer, no credentials). Bridge: `show.tvmazeId` → TVMaze `externals.imdb`/`thetvdb` → TMDB `/find` → `/videos` → earliest official YouTube Trailer (Teaser fallback). 5-checkpoint plan ready (CP1 bridge lib → CP2 in-memory parallel-map cache (hits+misses) → CP3 TrailerCard facade→iframe → CP4 gate `curVal===S0E0` + render + two-layer centering → CP5 verify). Do NOT start building until Alborz re-confirms.
