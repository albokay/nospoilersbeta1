...
> - **Round-3 (Alborz):** the show-room header's "with" is now `CANON.dark` (was greyblue; the group-room "with" line is unchanged). **Groups-only dashboard now centers vertically** — the clusters + "Create another watch group?" were pinned to the top when sparse; the page is a flex column for the non-group context and its body (`dashboardCenter`, flex:1) centers clusters+button as a unit (content taller than the viewport grows and the page scrolls — flex-item min-height auto, no clip). Group context untouched (page stays block flow; `clustersEl` extracted + rendered in both branches; edge tabs/sticky are position-fixed so DOM order is irrelevant).
