...
> - **1. BETA-USERS CUTOVER MIGRATION (separate, destructive-ish, needs a backup + go/no-go).** Move the 5 beta users' now-invisible **personal shelves** + **private-journal entries** into the groups-only world — **NOTHING may be deleted**. Not started. Private entries should surface as **drafts** (CP6). See `[[project_social_onboarding_arc]]` locked answer #2.
...
> - **3. SMALLER FOLLOW-UPS:** (a) `RatingCaptureModal` still shows **dice dots** — inconsistent with the map's stars; swap to `StarFace` (quick, one file, both platforms). (b) The **onboarding flow (desktop AND mobile) has no "hi, it's…" field**, so its invitees start with the inviter's handle (the invite modal/sheet has the field + accept-time seeding works — wire a field into Screen 2 of both if wanted). (c) **Progress-aware onboarding prompts** (haven't-started vs further-along) — spec-deferred. (d) **Naming the person who invited YOU** beyond the "hi, it's…" seed — the gear rename covers it; no dedicated surface. (e) **Mobile group rooms have no `GroupRoomSticky`** (predates the arc), so the CP3 bootstrap "we went ahead…" note shows only if the group is ever opened on desktop (`ns_onb_group` is set by mobile onboarding too). (f) `MobileSearchSheet`'s personal mode is dead code since mobile CP2 (no consumer) — clean up whenever.
