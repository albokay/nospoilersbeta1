...
> **2026-05-09 — Latest landed: password reset flow is live (`/reset-password` + Forgot password? + supabase implicit flow); v2 compose page rebuilt (textarea ruled paper on white, watch-progress is now an updater, single-select destination pills with radio dots + dynamic explainer); thread-view Duplicate/Delete now redirect.** Earlier today: `/v3/journal` = wholesale duplicate of live ProfilePage shipped, "Duplicate to…" thread-clone affordance shipped. See §7 arcs (top of recent-work list) for plumbing + rationale.
