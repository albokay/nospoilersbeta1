...
> - **2. POOL PAGE LEAKS GROUP PROPOSALS (flagged during the mobile arc; BOTH platforms, pre-existing since desktop CP1 2026-07-06).** The pool page (`/pool/:username` + `/m/pool/:username`) reads ALL progress rows via `get_public_progress` (no `in_pool` filter — `20260511_v2_get_public_progress_v3.sql` is `WHERE p.user_id = target_user_id` only). Proposing a show creates a quiet S0E0 `in_pool=false` row (`ensureProgressRow`), so a mere group proposal surfaces under "wants to watch these shows:" on the proposer's pool page. Fix options: filter in the RPC (migration) or client-side on both pool pages. Alborz aware; decide + fix when convenient.
