  ...
  if (compactLabel) {
    return (
      <>
        <button
          className="btn"
          style={{ whiteSpace: "nowrap", background: "var(--progress-bg,#bdd4de)", color: "var(--progress-fg,#355eb8)", border: "2px solid var(--progress-bg,#bdd4de)", fontSize: 12, padding: "5px 9px", lineHeight: 1.2, fontWeight: 700 }}
        ...
        {mobileOpen && (
          <Modal onClose={() => setMobileOpen(false)}>
            ...
            <select
              ...
              style={{ background: "var(--progress-bg,#bdd4de)", color: "var(--progress-fg,#355eb8)", border: "2px solid var(--progress-bg,#bdd4de)", width: "100%", height: 40 }}
              size={1}
            >
              {groups.map((g) => (
  ...
  return (
    <>
      <select
        ...
        value={selectedId}
        onChange={onSelect}
        style={{ background: "var(--progress-bg,#bdd4de)", color: "var(--progress-fg,#355eb8)", border: "2px solid var(--progress-bg,#bdd4de)", fontWeight: 700, fontSize: 12, textAlign: "center", textAlignLast: "center" }}
      >
        {groups.map((g) => (
          <optgroup key={g.season} label={`Season ${g.season}`}>
            {g.episodes.map((ep) => (
