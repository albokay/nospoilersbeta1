...
export default function SocialOnboarding({ onDone }: { onDone: (groupId: string | null) => void }) {
  ...
  async function publishAndInvite(data: {
    ...
    // Perf (2026-07-26): the invite mints run in PARALLEL (they were a
    // serial per-friend chain), and the whole batch overlaps the seed
    // entry's insert — together they were most of the visible "posting…"
    // wait. bootRef keeps its resume-not-duplicate semantics: every
    // completed step still lands in boot before anything can throw past it.
    await Promise.all([
      Promise.all(
        invitees
