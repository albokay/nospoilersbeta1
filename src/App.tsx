        ...
        <>
          ...
          {isHomepage && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", paddingTop: isMobile ? 48 : 56, paddingBottom: 24, zIndex: 95, position: "relative", width: "100vw", marginLeft: "calc(-50vw + 50%)" }}>
              <div style={{ transform: `translateX(${isMobile ? 8 : 13}px)` }}><SidebarLogo scale={isMobile ? 0.6 : 1} /></div>
              ...
              {!user && (
                <div style={{ marginTop: 36, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 12, color: "var(--dos-user)", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                    Breaking Bad (Demo Forum)
                  </span>
                  <button
                    className="btn"
                    onClick={() => openShow("bb")}
                    style={{ fontSize: 15, padding: "8px 28px", borderRadius: 9999, whiteSpace: "nowrap", color: "var(--dos-user)", borderColor: "var(--dos-user)" }}
                  >
                    Enter the demo →
                  </button>
                </div>
