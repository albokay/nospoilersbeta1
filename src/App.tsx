...
export default function App() {
  ...
  // This useAuth() call is unconditional (above every early return) so the
  // hook count stays fixed per render — see §6 item 19.
  ...
  // ── Mobile routing (the FLIP, 2026-07-03): phones get the /m app. ────────
  // Desktop-shaped paths on a <768px viewport redirect into /m. Passthroughs:
  //   • "m"              — the mobile app itself.
  //   • "reset-password" — recovery emails land here with tokens in the URL
  //                        hash; the page is narrow-friendly (420px card).
  //   • "invite", "show-room", "group-invite", "pool" — each has its own
  //     viewport fork below that preserves params (?entry=, tokens); letting
  //     this generic redirect swallow them would strand deep links on /m.
  // Admins stay exempt so the desktop UI remains QA-able from a phone; the
  // race guards (return null) keep an admin's first paint from redirecting
  // before their profile resolves. location.hash rides along so a Supabase
  ...
  // The admin exemption is for QA-ing the DESKTOP site from a phone's
  // browser. Inside the INSTALLED app (standalone, no browser chrome) the
  // desktop pages draw under the notch/status bar — so there admins take
  // the phone app like everyone else (Alborz 2026-08-18).
  const adminExempt = isAdmin && !isStandalone();
  ...
  if (pathParts[0] === "pool" && pathParts[1]) {
    ...
    // 2026-09-08 pt 4: the desktop profile PAGE is retired (Sidebar is
    // friends-only — no signed-out profile viewing); in-app name clicks open
    // the friend-profile DRAWER instead, so a stray desktop /pool link just
    // lands on the dashboard (its auth guard bounces signed-out visitors
    // home). The pool rendering survives only in the invite arrival, which
    // mounts PublicDashboardPage directly with the invite prop.
    return <Navigate to="/dashboard" replace />;
  }
...
function AppShell() {
          ...
          <div style={{ position: "relative", display: "inline-block" }}>
            ...
            >
              {(showProfile || showJournal || showLegacyProfile)
                ? <><BookOpen size={16} color={CANON.cream} style={{ flexShrink: 0 }} /><span className="profileChipLabel" style={{ fontWeight: 700, color: CANON.cream, display: "inline-flex", alignItems: "center", gap: 6 }}>you are <SidebarAvatar userId={user?.id} username={username ?? undefined} size={18} />{username}</span></>
                : <><BookMarked size={16} color={CANON.cream} style={{ flexShrink: 0 }} /><ArrowLeft size={14} color={CANON.cream} style={{ flexShrink: 0 }} /><span className="profileChipLabel" style={{ fontWeight: 700, color: CANON.cream }}>go to your journal</span></>
              }
            </button>
            {pillBadge === "green" && (
  ...
  // Mobile routing lives in the top-level <App> (the FLIP gate): non-admin
  // phone visitors are redirected into /m before AppShell ever renders, so
  // there's no gate here. Only admins-on-mobile reach AppShell at <768px.
                  ...
                  {(["first", "rewatch"] as const).map(choice => (
                    <div key={choice} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", fontSize: 14 }} onClick={() => setWatchStatusChoice(choice)}>
                      <div style={{ width: 20, height: 20, borderRadius: "50%", flexShrink: 0, border: "none", background: CANON.cream, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {watchStatusChoice === choice && <div className="radio-dot" style={{ width: 10, height: 10, borderRadius: "50%", background: CANON.personal }} />}
                      </div>
